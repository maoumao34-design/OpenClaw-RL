...
# See docs/metaclaw_migration_plan.md "方案：按题成组 + 1/N advantage 缩放".
#
# A MetaClaw round stays N per-turn samples (each with its own real prompt and
# its own real response -- nothing is reconstructed), but the whole round is
# emitted as ONE group, and the advantage of every sample in it is divided by N
# downstream. Net effect: every round contributes exactly one round's worth of
# gradient regardless of how many turns it took, and a batch is always exactly
# `rollout_batch_size` complete rounds.
#
# Why not one flat trajectory sample (the 2026-09-03 first attempt, reverted):
# OpenClaw strips the assistant's reasoning when it replays history
# (`dropReasoningFromHistory` in openclaw/src/agents/transcript-policy.ts:174 --
# our provider matches none of the four preserve conditions in
# shouldPreserveReasoningContentReplay). So turn t is generated as
# `think_t + call_t` but turn t+1 only ever sees `call_t`. A single flat
# sequence has to pick one: keep think_t and every later turn's context is
# fabricated; drop it and turn t's own generation is misrepresented, so
# training that span teaches the model to emit tool calls without thinking.
# There is no third option -- an autoregressive sequence cannot express
# "generated, then removed from context". Per-turn samples are the only
# faithful representation, which is why this design keeps them.
#
# There is deliberately NO on/off switch, and it is gated on
# turn_data["metaclaw_round_mode"] (session_id starts with "metaclaw-"), so the
# Personal Agent Track is untouched.
...
fire_opd_task_new = (
    ...
    '            if not _is_verdict:\n'
    '                logger.info(\n'
    '                    "[openclaw-rl-metaclaw-round-group] session=%s turn=%d "\n'
    '                    "held (intermediate turn, no judge, no sample of its own) "\n'
    '                    "-- will be submitted together with this round verdict",\n'
    '                    session_id, turn_num,\n'
    '                )\n'
    '                return\n'
    '            # Verdict turn: fall through and fire the task as usual. The\n'
    '            # round-wide submission happens in _maybe_submit_ready_samples\n'
    '            # (openclaw_combine_api_server.py), which is where the held turns\n'
    '            # and this turn are built into a single group.\n'
    '\n'
