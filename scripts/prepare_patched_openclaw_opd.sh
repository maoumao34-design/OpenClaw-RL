...
tool_calls_log_new = (
    ...
    '                        _is_invalid_tool_use = True\n'
    '\n'
    '            # 规则 6（2026-08-31）：结构性无效的工具调用 -- 参数根本没法解析，\n'
    '            # 或者写类工具没给目标路径。跟规则 4 的区别很重要：规则 4 只在\n'
    '            # Personal Agent Track 生效（靠 student-hw-N- 这个 session id 形态\n'
    '            # 推出期望路径，MetaClaw session 拿不到，整段被跳过），而且它只在\n'
    '            # "path 存在但写错地方"时才判负；path 干脆缺失的情况两条都不覆盖。\n'
    '            # 真实数据（metaclaw_migration_20260831_154301）：day05/r10 模型输出\n'
    '            # 嵌套 JSON 时转义出错 -> SGLang 报 Failed to parse JSON part ->\n'
    '            # 下一轮把 validation error 当上下文喂回去 -> 模型开始反复输出\n'
    '            # {"name": "write", "arguments": {}}；day08/r1 连续 17 次同样的空参数\n'
    '            # write，单个 round 产出 23 个训练样本、跨多个 batch。首次那一发\n'
    '            # is_invalid_tool_use=False，规则 1a 只能拦住第二发起的逐字重复，\n'
    '            # 所以坏轨迹的起点始终没被标记。\n'
    '            # 这两条都是任务无关的必然错误，不依赖任何具体任务/工具集：参数\n'
    '            # 不是合法 JSON 的调用不可能被正确执行；写类工具没有目标路径就\n'
    '            # 无处可写。判定放在 tool 名字判断之外，对所有工具生效。\n'
    '            if _tool_args_raw:\n'
    '                try:\n'
    '                    _parsed_args = json.loads(_tool_args_raw)\n'
    '                except (TypeError, ValueError):\n'
    '                    _is_invalid_tool_use = True\n'
    '                    _parsed_args = None\n'
    '                if isinstance(_parsed_args, dict) and _tool_name in ("write", "edit"):\n'
    '                    if not _parsed_args.get("path"):\n'
    '                        _is_invalid_tool_use = True\n'
    '\n'
...
metaclaw_verdict_skip_new = (
    ...
    '                    # --- openclaw-rl-metaclaw-midround-reward (2026-08-31) ---\n'
    '                    # Record WHICH turn carries this round\'s verdict, set only\n'
    '                    # after _fire_opd_task actually ran. Two consumers in\n'
    '                    # _maybe_submit_ready_samples need this and neither can\n'
    '                    # derive it otherwise:\n'
    '                    #   1. force_drop must NOT discard held turns while a verdict\n'
    '                    #      is genuinely in flight (verdict path runs force_drop\n'
    '                    #      before the task completes), but MUST discard them on\n'
    '                    #      the infrastructure-failure path where no verdict is\n'
    '                    #      ever fired. "Are any tasks still running" cannot tell\n'
    '                    #      those apart -- intermediate tasks may be running in\n'
    '                    #      both cases.\n'
    '                    #   2. task.result() raising loses the opd_result entirely,\n'
    '                    #      so the only way to know the FAILED task was the verdict\n'
    '                    #      one (and therefore that no outcome is ever coming) is\n'
    '                    #      to compare turn_num against this.\n'
    '                    _mr = self._metaclaw_round.setdefault(\n'
    '                        session_id,\n'
    '                        {"held": {}, "outcome": None, "verdict_turn": None, "state": "pending"},\n'
    '                    )\n'
    '                    _mr["verdict_turn"] = _prev_turn_num\n'
    '            if session_done:\n'
...
train_until_day_init_new = (
    ...
    '\n'
    '        # --- openclaw-rl-metaclaw-midround-reward (2026-08-31, temporary) ---\n'
    '        # Per-session round bookkeeping for the mid-round-reward ablation (see\n'
    '        # docs/metaclaw_migration_plan.md "方案：中间轮次改吃本轮最终 checker\n'
    '        # 结果的消融实验"). One entry per MetaClaw round -- since 2026-08-19c\n'
    '        # every round IS its own session, so session_id is the round boundary\n'
    '        # and no cross-round inference is needed.\n'
    '        #   held         {turn_num: (turn_data, opd_result)} intermediate turns\n'
    '        #                waiting for this round\'s deterministic outcome\n'
    '        #   outcome      the round\'s checker score once known, else None\n'
    '        #   verdict_turn turn_num the verdict evaluation was fired for, else\n'
    '        #                None. None means "no verdict will ever arrive"\n'
    '        #                (infrastructure-failure path, which closes the session\n'
    '        #                via _send_session_close_only without ever sending a\n'
    '        #                verdict) -- that distinction cannot be recovered from\n'
    '        #                task state alone, because intermediate tasks can also\n'
    '        #                still be running on that path.\n'
    '        #   state        pending | succeeded | failed | no_verdict\n'
    '        # Entirely inert unless METACLAW_MIDROUND_REWARD=outcome.\n'
    '        self._metaclaw_round: dict[str, dict] = {}\n'
    '\n'
    '        self.app = self._build_app()\n'
)
