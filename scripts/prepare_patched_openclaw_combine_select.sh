...
opd_evaluate_head_new = (
    ...
    '                    # a tool observation. The round task is the FIRST user message\n'
    '                    # of this session (one session per round, see the driver), so\n'
    '                    # trim to it, let the helper find it, then splice the rest of\n'
    '                    # the conversation back on unchanged.\n'
    '                    _mc_msgs = turn_data["messages"]\n'
    '                    _mc_first_user = next(\n'
    '                        (k for k, m in enumerate(_mc_msgs)\n'
    '                         if isinstance(m, dict) and m.get("role") == "user"),\n'
    '                        None,\n'
    '                    )\n'
    '                    if _mc_first_user is None:\n'
    '                        _enhanced_messages = _append_hint_to_messages(\n'
    '                            _mc_msgs, _metaclaw_hint,\n'
    '                        )\n'
    '                    else:\n'
    '                        _enhanced_messages = (\n'
    '                            _append_hint_to_messages(\n'
    '                                _mc_msgs[: _mc_first_user + 1], _metaclaw_hint,\n'
    '                            )\n'
    '                            + _mc_msgs[_mc_first_user + 1 :]\n'
    '                        )\n'
