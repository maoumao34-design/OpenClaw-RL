...
opd_evaluate_head_new = (
    ...
    '            # --- openclaw-rl-metaclaw-hard-negative-precedence (2026-08-31) ---\n'
    '            # Set by the two forced-(-1) rules below. Reported out so that\n'
    '            # outcome mode can keep the override instead of overwriting it with\n'
    '            # the round result -- see _maybe_submit_ready_samples. Recorded here,\n'
    '            # where the rules actually fire, rather than re-deriving it from\n'
    '            # turn_data downstream, so the rule set has one owner.\n'
    '            _hard_negative = False\n'
