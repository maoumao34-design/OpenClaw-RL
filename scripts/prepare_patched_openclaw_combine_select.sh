...
opd_evaluate_head_new = (
    ...
    '            # ±1 never actually reached the training queue. Returning here,\n'
    '            # mirroring the metaclaw_round_mode/step-judge branch\'s own\n'
    '            # return shape below, submits eval_score as a plain RL sample\n'
    '            # instead (accepted=False -- no OPD hint distillation for this\n'
    '            # turn). This is a real scope reduction from the original intent\n'
    '            # (the `votes` list built above, when the hint is long enough,\n'
    '            # was meant to flow into the shared hint-acceptance code and\n'
    '            # potentially get OPD treatment) -- but that path has never once\n'
    '            # executed successfully (always crashed first), so this does not\n'
    '            # regress anything that was actually working. If OPD treatment\n'
    '            # for verdict hints is wanted later, do it explicitly (set\n'
    '            # `_skip_forced_negative_override = False` here and fall through)\n'
    '            # rather than reverting this return.\n'
    '            return {\n'
    '                "accepted": False,\n'
