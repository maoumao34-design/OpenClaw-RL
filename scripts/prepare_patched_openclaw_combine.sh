...
round_submit_new = (
    ...
    '\n'
    '        OpenClaw drops the thinking from an assistant turn before replaying\n'
    '        it, so this is the form an earlier turn takes inside a later prompt\n'
    '        -- and therefore what has to be searched for when putting the\n'
    '        thinking back.\n'
    '\n'
    '        Cut at the CLOSING tag, not at a <think>...</think> pair: the\n'
    '        Qwen3-Thinking template puts the opening <think> in the generation\n'
    '        prompt, so a response begins in the middle of the thinking and\n'
    '        carries only the closer. Pairing on an opener that is never there\n'
    '        left the whole thinking inside the search string and made every\n'
    '        lookup fail -- measured 0 hits across every multi-turn round of\n'
    '        run 20260914_154812 before this was fixed.\n'
    '        """\n'
    '        _t = text_in or ""\n'
    '        _i = _t.find(_MC_THINK_CLOSE)\n'
    '        if _i >= 0:\n'
    '            _t = _t[_i + len(_MC_THINK_CLOSE):]\n'
    '        return _t.strip()\n'
    '\n'
    '    @staticmethod\n'
    '    def _metaclaw_generation_tail(prompt_text):\n'
    '        """Whatever the generation prompt appended after the assistant header.\n'
    '\n'
    '        For this template that is the newline and the opening <think>. The\n'
    '        backbone does not have it -- a replayed turn renders straight from\n'
    '        its structured form -- so splicing a full response back in without\n'
    '        it would leave a closing </think> with no opener, a shape the model\n'
    '        neither read nor wrote. Read it off that turn rather than being\n'
    '        hardcoded, so a template change cannot silently invalidate it.\n'
    '        """\n'
    '        _p = prompt_text or ""\n'
    '        _h = _p.rfind(_MC_ASSISTANT_HDR)\n'
    '        if _h < 0:\n'
    '            return ""\n'
    '        return _p[_h + len(_MC_ASSISTANT_HDR):]\n'
    '\n'
    ...
    '                )\n'
    '                continue\n'
    '            # Put back the tail the generation prompt supplied, ahead of the\n'
    '            # response, unmasked: it was prompt, not generation, and\n'
    '            # without it the spliced thinking has no opener.\n'
    '            _hdr_at = backbone.rfind(_MC_ASSISTANT_HDR, cursor, _at)\n'
    '            if _hdr_at >= 0:\n'
    '                _tail = self._metaclaw_generation_tail(\n'
    '                    _td.get("prompt_text")\n'
    '                )\n'
    '                _keep = backbone[cursor:_hdr_at + len(_MC_ASSISTANT_HDR)]\n'
    '                pieces.append((False, _keep + _tail))\n'
    '            else:\n'
