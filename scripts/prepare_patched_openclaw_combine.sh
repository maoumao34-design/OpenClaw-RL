...
midround_postloop_new = (
    ...
    '        if _METACLAW_MIDROUND_REWARD == "outcome":\n'
    '            _mr = self._metaclaw_round.get(session_id)\n'
    '            if _mr is not None and force_drop_without_next_state and _mr.get("verdict_turn") is None:\n'
    ...
    '                # for the loop above to drop, so they must be discarded here or\n'
    '                # they leak forever.\n'
    '                if _mr["held"]:\n'
    '                    logger.info(\n'
    ...
    '            elif (\n'
    '                _mr is not None\n'
    '                and _mr.get("state") == "succeeded"\n'
    '                and not _mr["held"]\n'
    '                and not pending\n'
    '                and not prm_tasks\n'
    '            ):\n'
    '                # Success path cleanup (2026-08-31 fix, CLI review). Deliberately\n'
    '                # NOT gated on force_drop_without_next_state: the verdict flush\n'
    '                # happens on the task-completion callback, which calls this method\n'
    '                # WITHOUT force_drop, so a force_drop-gated cleanup could never run\n'
    '                # on the normal successful path and every completed round leaked an\n'
    '                # entry (reward delivery was unaffected -- purely a state leak).\n'
    '                # The pending/prm_tasks emptiness checks are what make unconditional\n'
    '                # cleanup safe here: a judge task that finishes AFTER the verdict\n'
    '                # still needs _mr["outcome"] to inherit from, and while such a task\n'
    '                # is outstanding its turn is still in both dicts. Only once both are\n'
    '                # empty can nothing further arrive for this session.\n'
    '                self._metaclaw_round.pop(session_id, None)\n'
)
