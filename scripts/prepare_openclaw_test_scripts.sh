...
# ---------------------------------------------------------------------
# 08-11 七次修订（同一天，六次修订加了数数字 CoT + 反例之后，真实训练里
# 又复现同一类误判——P17：Turn 1 已经有 "3 fish x $1/day x 31 days ... =
# $93" 这种显式算式，数字（3/1/31/93）远不止一个，完全满足②"数到 >=2
# 个数字就算有过程"的豁免条件，Simulator 仍然回复"You only gave me one
# number"，跟提示词字面矛盾。②这条判断连续三轮（五次修订新增、六次
# 修订加码、这次又复现）都没能让 Simulator 稳定执行——不是文字精度问题，
# 是这条判断本身对当前 Simulator 不可靠。用户决定：整条删除②，不再
# 判断"是不是只有单个数字/只有答案没过程"，①（有没有真答案）通过之后
# 直接进入原③（AI 味重写判断，现在是这条唯一剩下的后续检查）。
#
# 已确认的取舍：Table 3 收敛判定的正则（satisfies_student 系列）完全不
# 检查"有没有过程"，只看 bold/编号列表/boxed 三个格式特征，所以这个
# 改动对 Table 3 收敛数字本身没有直接风险；但②是整条训练流程里唯一在
# 执行论文"必须包含完整步骤"这条要求的机制，删除后如果 policy 学会用
# 裸答案通过训练，不会被 Table 3 指标发现。用户主动接受这个取舍，先看
# 下一轮训练是否重新出现"裸答案"问题；如果出现，用户明确要求换成正则
# 等确定性方法识别，不再继续在这条 prompt 上加码给 Simulator 判断。
# ---------------------------------------------------------------------
python3 - "${SRC_DIR}/student_chat.py" "${DEST_DIR}/student_chat.py" <<'PY'
import sys
...
if text.count(old_bullet23) != 1:
    raise SystemExit(
        f"patch failed: expected exactly 1 occurrence of the bullet 2+3 block "
        f"in student_chat.py, found {text.count(old_bullet23)} (an earlier "
        "patch in this script may have changed -- update this patch)"
    )
new_bullet2_only = (
    '   - Otherwise: if it looks AI-like -- bold text, numbered lists, or "**Final answer**:" -- tell it to redo it in a more natural, casual way. Just ask it to fix the tone/formatting -- don\'t ask for more steps or more detail. Do NOT mention writing to the file in the same message. Only ask for a rewrite. If it\'s already natural-sounding with no AI-like formatting, no need to redo.\n'
)
text = text.replace(old_bullet23, new_bullet2_only, 1)

with open(dest_path, "w", encoding="utf-8") as f:
    f.write(text)
print(f"patched (Step 1: removed bullet 2 'only-answer-no-process' check entirely -- Simulator repeatedly failed to execute it correctly despite CoT+examples) -> {dest_path}")
PY

echo "已生成 openclaw-test 补丁: ${DEST_DIR}（model 字段兼容修复 + student_chat.py 去掉开放式 AI-like 兜底 + 单题异常容错 + Steps 真答案检测+AI味重写判断+写入核实（严格版）+ 角色串戏防护，只有答案没过程判断已移除，FIRST_MESSAGE_TEMPLATE 已撤销恢复官方原始措辞，homework-verification-gate 已移除，见 docs/issues_log.md 2026-07-23 / 2026-07-29 / 2026-08-03 / 2026-08-06 / 2026-08-07 / 2026-08-11）"
