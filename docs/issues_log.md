...
**待办：** 是否需要针对"edit 因为找不到精确匹配文本而反复失败"这个具体模式加一条新的确定性规则（比如：检测到 `edit` 工具返回的错误信息包含"could not find"/"no match"这类明确的失败标记时，不管 LLM 判官怎么判，直接强制打负分——这个思路上更接近"消息类工具需要真正送达"那条规则的精神，是可以通过 next_state 内容做确定性判断的，不需要理解语义），还是先观察这个模式出现的频率再决定优先级。
