...
3. 训练启动后第一时间查 `openclaw.log` 里有没有出现 `[execution-bias-fix] resolveSystemPromptContribution invoked` 这行确认日志——出现了才说明这个钩子真的被内置 sglang provider 调用了，没出现要立刻停下来排查（不要等训练跑完再看结果）
