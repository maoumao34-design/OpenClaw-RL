...
**决策（用户拍板）：** 先不回退版本，在当前 2026.6.9 上做定点修复。先验证这个改法能不能解决问题，如果不够彻底再评估要不要处理上面新发现的 retry 机制或版本回退。

**更新（实现阶段发现 `resolveSystemPromptContribution` 不是零接触方案，改用已验证可行的 `before_prompt_build` 机制）：** 深入到 `resolveProviderSystemPromptContribution`（`src/plugins/provider-runtime.ts:215`）→ `ensureProviderRuntimePluginHandle`（`src/plugins/provider-hook-runtime.ts:360`）的实际调用链后发现：这条路径是**按 provider 精确单一绑定**的（`resolveProviderRuntimePluginHandle({ provider: params.provider, ... })`，没有多插件合并机制），我们的 provider 是 `"sglang"`，已经被内置的 `extensions/sglang` 占用——想用这条路径必须直接改内置扩展本身，不是最初以为的"写独立插件、零接触官方源码"方案，纠正之前的判断。

**改用方案：** 复用项目里已经在真实训练中验证过可行的 `before_prompt_build` 钩子（`rl-training-headers` 插件用的就是这个机制）。这个钩子的返回类型（`PluginHookBeforePromptBuildResult`，`src/plugins/hook-before-agent-start.types.ts:28`）只有 `appendSystemContext?: string` 一个字段，只能**追加**、不能删除/替换已有章节——所以做法从"整段 override 去掉那一条表述"调整为"在系统提示词末尾追加一条消歧规则"：

```
Plain-text replies are not function calls: never wrap a plain-text reply in
<tool_call> tags. Only an actual tool invocation (name + arguments) uses
<tool_call> format. If you have already decided what to say and it does not
call a tool, output that text directly as your reply and stop -- do not
re-analyze whether it needs <tool_call> wrapping.
```

追加在提示词末尾（离生成最近的位置）通常比修改前面的静态章节影响力更强，效果上应该等价甚至优于"删掉那一条表述"这个原始设想。

**实现（新增独立插件，不修改任何官方文件，不属于"补丁副本"模式而是全新插件，因为没有官方源码可 diff）：**
- `scripts/openclaw-plugin-execution-bias-fix/{index.js, openclaw.plugin.json, package.json}` —— 静态插件源码，`enabledByDefault: true` + `activation: {onStartup: true}`（沿用 `prepare_patched_rl_training_headers.sh` 里确认过的"缺这两个字段插件加载器会跳过"这条经验）
- `train_with_services.sh`、`minitest_train_with_services.sh`、`smoke_train_with_services.sh` 三处都在 `rl-training-headers` 部署代码块之后追加了同样的部署步骤（复制到 `/usr/lib/node_modules/openclaw/dist/extensions/execution-bias-fix/` 系统插件目录 + `openclaw plugins enable execution-bias-fix`），保持跟已验证插件完全一致的部署方式
