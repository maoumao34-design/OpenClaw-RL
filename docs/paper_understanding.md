...
| 层次 | 组件 | 端口 | 职责 |
|------|------|------|------|
| Agent 运行时 | 真正的 `openclaw gateway run`（真实产品，含 workspace 插件） | 18789 | 给 Policy(4B) 提供实际的文件读写等工具能力：解析模型的工具调用请求、真正执行文件系统操作（对应论文原文"use OpenClaw to conduct their work"、产出 `homework1`/`homework2` 目录，见本文档第十三节引用）、把执行结果喂回模型下一轮对话 |
| RL 数据管道 | 官方仓库 `openclaw_combine_select_api_server.py`（论文作者自己写的训练数据采集代理，本身不执行任何工具调用） | 30000（RL 训练代理，内部再转发给真正的 SGLang Policy） | 下表列出的路由/buffering/PRM/样本组装职责 |
