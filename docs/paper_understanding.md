...
student_chat.py
  ...
  │
  └─② send_to_openclaw()
       POST port 18789（OpenClaw Gateway）
                │
             ├─ 转发给 Qwen3-4B（port 30000）生成 Policy 回复
                │
...
| **18789** | OpenClaw gateway（`openclaw_combine_select_api_server.py`）| `student_chat.py` 的 `send_to_openclaw()` |
| **30000** | SGLang Policy（Qwen3-4B rollout）| OpenClaw gateway 内部的 `_handle_request()` |
| **PRM URL** | SGLang PRM Judge（GPU 6）| OpenClaw gateway 内部的 `_query_judge_once()` |
