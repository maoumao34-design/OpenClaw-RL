...
此前"day09 起题题 timeout（TO=11）"是**块内出现过 `timed out` 就算**。按最终回复重数：

| day | 纯 timeout 文案 | 其中仍 `score>0` |
|---|---|---|
| 09 | 5 | **2**（r6、r8）|
| 10 | 6 | **3** |
| 11 | 7 | 0 |
...
**day09–10 是"真答对 / 超时但文件已在 / 超时且无文件"三者混合**，不是全废；
**day11 起超时占主导且不再留下可过检文件**，FC 归零、Acc 才整天塌掉。
