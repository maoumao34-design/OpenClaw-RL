...
**动机（2026-08-20 写的时候漏了一半，2026-08-20e 补全）**：两个独立的理由，不是一个：

1. **测量维度**：论文 Full 档"边训边考"方法学下，Acc./Compl. 是一个混合了训练全程不同权重状态的滚动平均数——前几天接近 base，后几天用的是训到一半的权重，一个数字里混了多种能力水平，没法单独回答"训练到底有没有提高能力"这个更窄的问题（这不是说这个混合数字"不对"或者"不该用"——它仍然是论文方法学要求的主口径，见下方里程碑一节的更正——只是这个数字本身回答不了"分离掉权重混合之后，训练效果单独看有多大"这个问题）。
2. **风险权衡维度（CLI 最早提这个方案时就讲了，写文档时被我漏掉，这次补上）**：训练继续得越久，越可能撞上"继续训练→继续吃退化的训练信号→训坏"这类问题（day12-14 那种超长 thinking 空转塌陷就是一个真实例子）。在某个 K 天就停止更新权重，本身也是一种止损手段——不是纯粹为了拿到干净的测量数字，也是给"万一训练信号已经开始变差，还要不要继续冒着训坏的风险往下训"这个决策留一个可控的开关。CLI 当时的原话是"若 K 小（5-10）冻结后 Acc/Compl 仍明显高于 base → 早期正信号有用，不是假繁荣；若 K 大（20-30）冻结后反而更差 → 支持'继续吃 −1 会伤模型'"——这条验证思路本身就依赖"K 是一个可以主动调小、及时止损的旋钮"这个设计意图，不只是测量工具。

CLI 提出的具体方案：把 30 天拆成两段，前 K 天正常训练+边跑边考，dayK+1 起权重冻结、但继续用同一套 harness 真实跑完剩余天数——冻结段的 Acc./Compl. 是能跟"训练前"直接对比的干净数字（对应理由 1），K 本身也是一个可以主动调小、观察到苗头不对就提前止损的参数（对应理由 2）。
...
## 里程碑：K=6 冻结实验——首次观测到训练带来的真实、可信的能力提升（2026-08-20c，2026-08-20d 更正出分口径）

**这是本次迁移第一个可信的"训练确实有用"的正面结果**，用的是当天早些时候实现的 `METACLAW_TRAIN_UNTIL_DAY` 功能（`commit 0882f69`）——训练 day1-6、day7 起冻结权重继续跑完剩余天数。

**出分口径更正（2026-08-20d，重要）**：初版记录把"Frozen 窗口"当成了这次实验的主结果，这是错的，已改正。**主结果必须是本趟 `metaclaw_rollout_driver` 全程 live 聚合的 Acc./Compl.**——这跟论文 MetaClaw Full/Table 1 的出分方式完全一致（同一趟训练运行实时聚合，不是训后另开 held-out 评测），是这次迁移从 08-14 起就确认的方法学主线，`094611` 那种全程 30 天的混合数字也是同一套方法学的产物，不是"不够干净"的东西。Train/Frozen 窗口的拆分**只是同一次 run 的分段诊断**，供分析"训练效果有没有被拆分开来看"用，**不能拿 Frozen 窗口替代全程聚合去当这次实验的成绩**——这两者回答的是不同问题：全程聚合回答"这趟训练按论文方法学算出来的 Table 1 式数字是多少"，分段诊断回答"排除掉'混合了多种权重状态'这个因素之后，训练效果能不能单独看出来"。
