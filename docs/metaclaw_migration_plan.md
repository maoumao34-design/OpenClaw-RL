...
## 🧭 项目定位（2026-09-11 用户拍板，此后不再讨论）

| | |
|---|---|
| **方法** | **Part II 的方法**（`benchmark/` 那套 agent 栈，`profile: coding` 全套工具）|
| **数据** | **Part I 规模的数据集**（`benchmark/data/metaclaw-bench/`，30 天 / 346 题）|
| **基线** | **一律以「按天隔离 K=0：Acc 41.1% / Compl 26.3%」为准** |
| **即** | **就是现在正在做的这套，方向不再变更** |
| **目标** | **通过迁移实验产出可用的专利，乃至论文** |

**"改回 Part I 方法"这个挂了多日的待讨论项就此关闭**（背景见下方「🔴 当前结论」与 2026-09-11 对 `metaclaw/` 的复核）。

### 目标改成专利/论文之后，什么跟着变

- **"对齐论文 Table 1 的数字"彻底不再是目标。**长期困扰本项目的"4B 零训练贴着 GPT-5.2"这个异常，**从此不需要被解释掉**——我们不声称复现它的任何一格。一切对照只跟自己的定版基线比。
- **判据从"复现得像不像"换成"主张站不站得住"**：需要的是**可主张的新颖点 + 干净的对照与消融**，而不是分数逼近某个已发表的值。
- **因此产出物的优先级要重排。**目前手里最接近"可主张"的三样：

  | | 成色 |
  |---|---|
  | **长度退化的机制发现**（`sum_of_sample_mean` 使一轮梯度贡献恒为 ±1 ⇒ 每 token 压力 ∝ `A/(N·T)` ⇒ 稀释惩罚最划算 ⇒ "多想"是最便宜的稀释手段）| **最成熟**：有配对权重探针的量化证据（中位数 4.2×、P90 10.1×）、有 `p_EXEC` 不变的反证排除了"策略漂移"这个竞争解释、有明确干预手段。**这是一条完整的"机制 + 证据 + 干预"链** |
  | **轮级信号到 turn 级样本的归属**（round-group + 1/N；轮级 hint 不得贴到中间 turn）| 有实现、有测试，**缺一次干净的对照实验** |
  | **中间步骤的第三条路**（借 `toolcall-rl` 的 step token 区间，避开 turn 级 ground truth）| **仅设计，未实现**；且 09-03 那次 96/120 轮丢弃的真实成因仍未查明 |

- **一个直接后果**：跑出"比基线高的分"是必要条件，但**不再是唯一交付**。一个有机制解释、有量化证据的**失效模式**，其可主张性不低于一个更高的分数。
...
## ✅ 已决定并实施（2026-09-07）：全面改为按天，按题模式已删除

**MetaClaw 全程按天隔离，天内靠压缩把上下文摁在上限以内。按题隔离是当初迁移 OpenClaw-RL 方法时带进来的产物，不是 MetaClaw 的做法。**

**改动理由**：08-19c 引入按题的原始理由是"共享 transcript 会让早轮的超长回复撑爆后续轮次的上下文"，**已被单变量 A/B 实测证伪**——同一个零训练模型按天反而更高（day01–17：**49.9% / 36.3%** vs 按题 **41.4% / 21.0%**）。担心的溢出没发生，因为 OpenClaw 自己的压缩吸收掉了。

**已删除**（`METACLAW_SESSION_SCOPE` 开关连同 round 分支整体移除）：
- session id 的 `-{group_id}-{round_id}` 后缀 → 改为一天一个 `metaclaw-{test_id}`
- verdict 的 `session_done` 恒 `True` → 改为 `is_last_round`
- infra 失败时每轮都发 `_send_session_close_only` → 改为仅最后一轮

**⚠️ 刻意保留，勿在后续清理中误删**（已加代码注释 + 测试守护）：
- **`metaclaw-` session 前缀**——代理靠 `_METACLAW_SESSION_RE` 匹配它做 round-mode 分派，删了整条分派就断，跟命名无关
- **`_FC_DIR_MODE_NOTE`**——讲的是文件名**格式**（别照抄示例日期），跟会话粒度正交
- **按题成组 + 1/N**——折叠逻辑是 `t < turn_num` + 每轮弹出，本来就 scope 无关

**⚠️ 一个常见误解，在此澄清**：`check_filename.py --dir --min-count` 的累计阶梯问题**按天不会消失**。工作区是 per-test（每天一份），跟 session 粒度无关；checker 数的是当天目录里累计的合规文件数，早轮少写一个后面永远追不平。**按天只是缓解**（共享 transcript 让命名风格一致，落后概率低），**结构性不公平仍在**。该问题**从未打过补丁**，所以也没有可删的东西。

**验证**：`py_compile` + `bash -n`；新增 `scripts/tests/test_metaclaw_day_scope.py` 13 项断言（三处站点一致、开关确实消失、三个保留项健在）；**非空洞性双向验证**——把 `session_done` 改回恒 `True` → 挂在"仅最后一轮"；把 `_FC_DIR_MODE_NOTE` 改名 → 挂在保留项检查。均已还原。

**⚠️ 三个后果，必须记住：**
1. **现有 K=0 基线（34.4% / 12.1%）是按题跑的，已失去可比性。**新基准必须是按天的 K=0（`20260907_112320`），**该趟尚未跑完**——在它出最终数之前，项目处于"没有完整基线"的状态
2. **`--max-tokens-per-gpu 32768` 尚未确认。**按天后 prompt 显著变长，rollout 侧已是 65536 但训练侧仍是 32768。**开训前必须先从 K=0 日志统计每轮 prompt token 分布（P50/P90/max）**，超了要先调
3. 所有按题的历史跑分**无法用新代码复现**（它们本已因口径更正作废，此处仅作记录）
...
### 📈 实测（2026-09-15）：退化的**放大**曲线，逐 step

`20260914_181842`，指标 = `response_text` 上 100 字符非重叠 shingle 的 `dup_frac`，按 step 分组：

| step | n | 中位 | P90 | **%>0.1** | %>0.8 |
|---|---|---|---|---|---|
| **≤9** | ~200 | 0 | 0 | **0%** | 0% |
| 10 | 42 | 0 | 0.48 | **17%** | 7% |
| 11 | 70 | 0 | 0.74 | **21%** | 10% |
| 12–14 | — | 0 | 0.86→0.95 | **27→44%** | 20→41% |
| 15 | 117 | 0.95 | 0.99 | **85%** | 80% |

**这条曲线要分成两半读，它们的证据地位完全不同：**

| | 状态 |
|---|---|
| **起病**：0% → 17%（step9→10）| ⬜ **无解释**。step≤9 是**精确为零**，不是"低" |
| **放大**：17 → 21 → 27 → 44 → 85 | ✅ **单调递增，六个点** |

> ⛔ **「稀释压力让输出逐渐变长」已于本日早些时候撤回**，本测量再次否定它：
> step≤9 完全没有重复抬头，不存在渐进过程。
>
> ✅ **但「训练在退化数据上会放大退化」被这条曲线直接支持**，而且不是靠机制推演。
> **step10 时中位数还是 0——系统当时有机会恢复。**（原文此处引用的 "+7.1pt" 已于 2026-09-17 更正：逐日看是两个离群日撑起来的，且 Compl 精确为 0，见上方「⛔ 更正」。）
> 它没恢复，因为每一步都在训练上一步产出的退化。

**主张因此收紧为：**

> **训练在退化数据上不「制造」退化，但它「放大」退化——把一次 17% 的扰动变成 85% 的整趟崩溃。**

### ❌ 阴性结果：step10 那一批里找不到触发器

**记下来是为了防止两周后有人重新去找一条不存在的轨迹。**

预设假设是「某个轮次思考已在打转却恰好通过 checker，于是 `advantage=+1` 直接强化了重复」。**逐条查完，不成立：**

| group | 约 round | reward | masked tok | turns | dup_max |
|---|---|---|---|---|---|
| 160 | day08 r4 | +1 | 2506 | 2 | **0** |
| 162 | day08 r5 | +1 | 2174 | 2 | **0** |
| 164 | day08 r6 | +1 | 2694 | 2 | **0** |
| 166 | day08 r7 | −1 | 1320 | 1 | **0** |
| 168 | day08 r8 | −1 | 5665 | 2 | **0** |
| 170 | day08 r9 | −1 | 920 | 1 | **0** |
| 172 | day08 r10 | +1 | 4916 | 3 | **0** |
| 174 | day08 r11 | **+1** | **8113** | **10** | **0** |

- 构成 **5 正 / 3 负**，不是全负批
- **没有任何 `+1` 且 dup 偏高的样本——正样本 dup 全为 0**
- 第一条 think ≥50k 在 19:33 day09，**在 step10 权重同步之后**，不在这批里

g174（+1、10 turn、8k token）是这批里最显眼的，但**「长且成功」离「触发重复循环」还很远**，不把它包装成答案。

**还没看过的唯一维度是优化器动力学**：step 8/9/10/11 的 `grad_norm`。09-09 那趟崩前是 1.52→0.82→0.58（在降）；若这次 step10 附近是尖峰，那是另一回事。

### ❌ 第二个阴性结果：起病处**没有任何离散异常**

step10 的优化器标量（`20260914_181842`，全 16 步）：

| step | 时间 | grad_norm | pg_clipfrac | ppo_kl | lr |
|---|---|---|---|---|---|
| 0–3 | 18:32–18:50 | 2.27 / 2.45 / 2.38 / 2.20 | 0 | 0 | 1e-5 |
| 4–6 | 18:55–19:04 | 3.37 / 2.88 / **4.32** | 0 | 0 | 1e-5 |
| 7–9 | 19:08–19:20 | 3.07 / 2.99 / 2.43 | 0 | 0 | 1e-5 |
| **10** | **19:29** | **2.59** ← 不是尖峰 | 0 | 0 | 1e-5 |
| 11 | 19:59 | 2.61 | 0 | 0 | 1e-5 |
| 12–15 | 20:49–23:45 | 3.98 / 5.95 / 5.61 / **8.39** | 0 | 0 | 1e-5 |

**两条互相独立的线索都指向同一件事：**

| 线索 | 结果 |
|---|---|
| step10 的批次构成（上一节 Q2）| 正常：5 正 3 负，正样本 dup 全 0 |
| step10 的优化器标量 | 正常：`grad_norm` 2.59 ≈ step9 的 2.43；clipfrac / kl 恒 0；lr 恒定 |

> ⛔ **不存在「那条触发轨迹」，也不存在「那一次异常更新」。停止找触发器。**

**更可能的解释（假设，非结论）**：step 0–11 的 `grad_norm` 一直在 2.2–4.3、lr 1e-5，
十一步累积的权重位移是实打实的，**但没有哪一步特殊**。输出层面的 0%→17% 是一次
**阈值穿越**，不是一次跳变。这同时解释了「输出突变」和「标量全正常」。

#### 两个佐证

**① `grad_norm` 后段与退化曲线是同一个形状**

```
grad_norm   2.59   2.61   3.98   5.95   5.61   8.39
退化比例     17%    21%    27%    44%     —     85%
            step10   11     12     13     14     15
```

**同步上升 → `grad_norm` 是后果指示器。**退化样本产生更大的梯度，更大的梯度又推得更远——
**这是同一个正反馈回路在梯度空间的显影**，与输出空间的 17→85 是一回事的两个视角。

**② 墙钟间隔独立确认了时点**

```
step 0→10   每步 3–9 分钟
step 10→11  30 分钟   ← 3 倍
之后        50 / 41 / 35 / 100 分钟
```

rollout 在 step10 之后立刻变慢（response 变长的直接体现）。**起病时点落在 step10 与 11 之间，
这条独立于 `dup_frac`。**

#### ⚠️ 熵：数据没拿到，但 CLI 给的理由是错的

CLI 报告 `entropy_loss` 全程 `0.0`，并判断"因为 `--entropy-coef 0.00`，所以只是占位符"。
**理由不成立**：

```python
# slime/backends/megatron_utils/loss.py:1003
...
**代码是刻意为监控计算真实熵的。** 而 `0.0` 这个值本身不可信——它是 8 个样本 per-token
平均熵之和，正常该是几个 nat 的量级；**step 0 是 base 权重、当时正产出 67% 正确且多样的
答案，熵不可能为 0**。同一行 `grad_norm` 有两位小数，也不像被格式化截掉。

**→ 是取数取错了，不是没有数。** 待 CLI 贴一整行 `step N: {...}` 原文（可能是
`grpo_entropy_loss` / `opd_entropy_loss` 之类的键名抽错）。⬜ **未完成**

#### 这反而让 gate 的定位更稳

> **训练期的任何标量都预测不到起病**——`grad_norm` / `clipfrac` / `ppo_kl` / `lr`
> 在起病时全部正常。

**所以正确的干预类别就是「对输出反应」，而不是「对训练信号预警」。**
gate 从"兜底手段"变成了"唯一可行的那一类"。

另记一条：`--entropy-coef 0.00` **且** `--kl-loss-coef 0.0` ——
**没有任何东西把策略拉住**，既不拉向高熵也不拉向 base。纯奖励最大化跑十几步，
**熵单调下降是预期行为而非异常**。若真实熵数据拿到且确实在降，"起病"就不再神秘，
只是"降到了某个点"。

### ⛔ 更正（2026-09-17）：`+7.1pt` 站不住，而且 checker 会白送 +1

**这一节取代此前"day01–07 相对基线 +7.1pt，day08 起被 timeout 打穿"的说法。**

#### 一、逐日拆开，+7.1 是两天撑起来的

| day | 基线 Acc | 训跑 Acc | ΔAcc | | day | 基线 Acc | 训跑 Acc | ΔAcc |
|---|---|---|---|---|---|---|---|---|
| 01 | 68.3% | 60.0% | **−8.3** | | 05 | 88.5% | 87.2% | −1.3 |
| 02 | 80.3% | 71.2% | **−9.1** | | 06 | 43.6% | 43.6% | 0 |
| 03 | 33.3% | 62.5% | **+29.2** | | 07 | 36.4% | 83.0% | **+46.7** |
| 04 | 66.7% | 55.0% | **−11.7** | | | | | |

**7 天里 5 天 ≤ 0**，聚合出来的 +7.1 **全部来自 day03 和 day07 两天**。

**而 Compl 在 day01–07 是 35.1% vs 35.1%，精确为 0。**

> **这不是"广泛的提升"，是两个离群日拉起来的平均。**
> 原来那句"前半段有效果"**降级为：逐日看更像噪声，未确立**。

#### 二、`--dir --min-count` 的**正方向**缺陷：零写入也能判 +1

本文档早就记过这个 checker 的**负方向**（早轮欠账 → 本轮做对也判 −1）。
**正方向从未记录，而它更有害。**

`benchmark/src/infer/infer_cmd.py:608` 的 `_run_file_check` 在 workspace 里跑 checker，
**只数目录里已有的合规文件，完全不看最终回复**。按天共享 workspace ⇒ 计数跨轮累计。

**实证（`20260914_181842` day09）：**

| 轮次 | 最终回复 | 判分命令 | `official_score` | 真实情况 |
|---|---|---|---|---|
| r6 | `idle timeout` | `--dir day09/ --ext md`（≥1）| **1.0** | 超时**前**已 `write day09/20260914_risk_assessment.md`（mtime 19:47:49）→ 文件在，判过 |
| r8 | `idle timeout` | `--dir day09/ --ext json --min-count 4` | **1.0** | **本轮写入之前目录里已有 4 个合规 json**（19:34 / 19:43 / 19:44 / 19:47，均来自更早轮次）→ **本轮即使零写入也过线** |

> **r8 是"完全没写文件也能判对"的确证。**
> `agent_succeeded=True` 只表示 CLI 进程没崩，不等于这轮答完了。

#### 三、后果：奖励信号被污染的方向，恰好是强化退化
...
**这不是"评测数字虚高"那么简单，是把错误的 +1 喂进了优化器。**

而且**污染是不对称的**：训跑的超时轮次远多于基线，而超时轮正是靠残留文件白拿 +1 的那批
——所以**训跑在有残留文件的日子会显得比实际好**（day09 的 +23.9 就是这么来的）。

#### 四、timeout 计数此前偏松（本小节 2026-09-17 二次更正）

原先的 TO 列是**块内出现过 `timed out` 就算**，把邻题的污染也算了进来，看起来像
"day08/day09 起几乎全天 timeout"。**按本轮最终回复重数：**

| day | 原先记的 | **真实 idle timeout** | **另：空回复** |
|---|---|---|---|
| **08** | 1 | **0** | **r8** |
| 09 | 11 | **5**（r1,r2,r3,r6,r8）| r11 |
| 10 | 11 | **6** | r13 |
| 11 | 10 | **7** | 0 |
| 12 | 11 | **8** | 0 |
| 13 | 11 | **6** | 0 |
| 14–16 | ≈全天 | 略少几题 | — |
| 17–21 | 全天 | 基本就是全天 | day19 r3 |

**三处要改口：**

1. **day08 的 timeout 数是 0。** 那天真正异常的是 **r8 空回复**，不是超时。
   → **"day08 起被打穿"是错的，onset 在 day09。**
2. **空回复是另一类失效**（day08 r8 / day09 r11 / day10 r13 / day19 r3），
   不该并进 timeout 计数。它对应「已知未修」里那条
   "空回复 turn 变 2~3 token 样本，官方守卫因 `response_text` 恒含 `<|im_end|>` 形同虚设"。
3. **后段也不是 100% timeout**：day13 只有 6（比 day12 的 8 还少，**非单调**）。
   跑得完的那些多半是**短答 MCQ**——答案短，生成不到上限就结束了。

**day09–10 仍是"真答对 / 超时但文件已在 / 超时且无文件"三者混合**，不是全废。

> ⬜ **一个观察，不下结论**：day08 r8 的空回复**比 onset 早一天**。
> 是前兆还是无关，目前没有证据区分。

#### 五、⬜ 由此产生的新假设（未验证）

> **步骤 10–12 的那些"正样本"，会不会本身就是超时/退化轮靠残留文件白拿的 +1？**

若成立，放大阶段的机制就是两段的：

| 阶段 | 机制 |
|---|---|
| step 10–12（还有 2/1 个正样本）| **退化轮拿到假 +1 → 直接强化退化** |
| step 13–15（0 个正样本）| 残留文件耗尽 → 全负批 → 无对比方向 |

**测法**：把 step 10–12 里 `reward=+1` 的样本挑出来，看它们的最终回复是不是 timeout、
`dup_frac` 是不是偏高。**这是目前最该查的一条。**

### 🔑 反解：`grpo_pg_loss` 精确编码了每一批的正负构成（2026-09-16）

**推导**：`clipfrac ≡ 0` ⇒ ratio ≡ 1；advantage 在样本内恒为 ±1；`sum_of_sample_mean`
于是让每个样本**精确贡献 ∓1**；再除以 `num_samples = 8`。

```
grpo_pg_loss = (n_neg − n_pos) / 8
```

**验证**：step10 是 −0.25 → `n_neg−n_pos = −2` → 5 正 3 负。
**CLI 在批次构成那一问里独立数出的正是 5 正 / 3 负。推导成立。**

| step | grpo | 正/负 | step | grpo | 正/负 |
|---|---|---|---|---|---|
| 0 | 0 | 4/4 | 8 | −0.25 | 5/3 |
| 1 | −0.5 | 6/2 | 9 | +0.25 | 3/5 |
| 2 | +0.25 | 3/5 | 10 | −0.25 | **5/3** |
| 3 | −0.25 | 5/3 | 11 | +0.5 | 2/6 |
| 4 | 0 | 4/4 | 12 | +0.75 | 1/7 |
| 5 | −0.25 | 5/3 | **13** | **+1.0** | **0/8** |
| 6 | −0.5 | 6/2 | 14 | +0.75 | 1/7 |
| 7 | 0 | 4/4 | **15** | **+1.0** | **0/8** |
...
**零个正样本时 GRPO 没有任何对比方向**——批里每个 token 被均匀压低，
而"这些都别做"**不含任何关于该做什么的信息**。

这个问题**一直躺在「已知未修」清单里**（"全负批：要 `n_samples>1` 才有组内比较"），
现在它有了逐点对齐的证据。
...
| 参数 | 值 | 后果 |
|---|---|---|
| `--n-samples-per-prompt` | **1** | 每题只答一次 → **组内无可比对象** |
| `--disable-rewards-normalization` | 开 | advantage = 原始 ±1，**不减组均值** |

**而且它连带关掉了官方自己的准入控制**（`slime/ray/rollout.py:407`）：

```python
def _drop_constant_reward_groups(self, samples):
    ...
    if self.args.advantage_estimator not in ["grpo","gspo"] or not self.args.rewards_normalization:
        return samples          # ← 我们这条路直接从这里返回，该机制从不生效
```

> **官方 GRPO 本来就有「一组里学不到东西就整组丢掉」的机制，
> 而我们的配置把它关着。** 这和 MetaClaw 的 support/query 排除是同一类东西。

### ⚠️ 部分收回：「OPD 主导驱动起病」

**成立**：起病前 `|opd_loss|` 是 `|grpo_pg|` 的 2–4 倍。
**09-11 说的「OPD 弱一个数量级」是错的，该纠正站得住。**

**不成立**：`|diff|_mean` 全 16 步都在 **0.985–1.000**，而这 16 步跨越了
**从 base 干净输出到 85% 纯复读**的全过程。

> **一个在模型从正常变成完全退化的过程中纹丝不动的量，不是在测模型的状态。**

查计算方式（`hint_opd_loss.py:420`）：`w = softmax(ell_old|S_t)` 就是重归一化的旧概率、
集中在 top-1，所以该量 ≈ **top-1 token 上的 |teacher − student|**。teacher 就是同一份权重
加了个 hint——**"往长 prompt 里插一段指令"的扰动量级大体恒定，与模型好坏无关**。

**推论：OPD 是近乎恒定幅度的扰动，不是分级的纠错信号。**
恒定的力解释不了阈值穿越，该假设**削弱**。

**起病后主导权反转**：`grpo_pg` 涨到 +1.0（全负批），`opd_loss` 稳在 −0.9 —— **起病后是 RL 主导**。

### 🚧 gate 判据：裸 flag 不能用，子信号可以

健康窗（step ≤9，分母 = 全部 200 个 MAIN turn，shadow 100% 覆盖，**不是有偏子集**）：

| gate 判据 | 健康窗命中 | 可用性 |
|---|---|---|
| `is_invalid_tool_use=True` 原样 | **15%**（30/200）| ❌ 太凶，明显瘦 batch |
| **`max_sentence_copies ≥ 3`** | **2%**（4/200）| ✅ 可接受 |

那 30 条里 **21 条 `copies=1`**，根本不是重复问题，是 `is_invalid_tool_use` 里其它条款被触发。
**→ gate 要用句重复子信号，不要用复合 flag。**

### 🧪 实验设计（对照组已经有了）

`20260914_181842` 的 `0 → 17 → 21 → 27 → 44 → 85` 就是**无 gate 的对照曲线**。加 gate 跑一趟，比同一条曲线：

| 结果 | 结论 |
|---|---|
| 退化比例**起病后走平或衰减** | ✅ 是数据在放大 —— 主张成立，gate 有效 |
| **照样一路涨** | ❌ 是权重进了坏盆地，与训练数据无关 —— **主张被证伪**，换方向 |
...
| | 主张 | 评价 |
|---|---|---|
| **1** | **实证展示放大机制**：论文从元学习目标上论证"不该这么做"，我们有**逐 step 的放大曲线**（0→17→21→27→44→85）。**注意：只能主张「放大」，不能主张「制造」——起病无解释** | **最有价值**：曲线是量出来的，不是推出来的 |
...
## 🔬 因果分析（2026-09-10）：为什么训练会让模型"多想"而不是"改错"

> ## ⛔ 本节的主因诊断已于 2026-09-15 被实测推翻
>
> 本节主张：`sum_of_sample_mean` 让一轮的梯度贡献恒为 ±1 ⇒ 每 token 压力 ∝ `A/(N·T)`
> ⇒ **稀释惩罚最划算 ⇒ 多想是最便宜的稀释手段**。
>
> **`20260914_181842` 这趟（第一次 (a) 真正跑起来、且有基线对照）把它否掉了：**
>
> | 观测 | 稀释叙事预测什么 | 实际 |
> |---|---|---|
> | 长度随 step 的形状 | **缓慢爬升**（梯度压力是连续温和的）| ❌ **step10 一步骤跳**：step≤9 中位 0.6–2k，step10 起 p90 直接 ~30k |
> | 超长内容是什么 | 更多、更细的推理 | ❌ **88–96% 字面重复**（100 字 shingle dup_frac 0.879 / 0.949 / 0.880），同一句空转，最高一句复读两三千次 |
>
> **那不是"模型选择多想"，是退化循环**——采样动力学里的吸引子，不需要长度奖励来解释。
> **我把一个能自圆其说的机制套在了数据上，套得太紧。**
>
> **本节还能活下来的，只剩一句弱得多的**：循环一旦发生，`sum_of_sample_mean` 把那一个 -1
> 摊薄到十几万 token 上，**推离循环的力很弱**。那是**维持**机制，不是**触发**机制。
>
...
| | base | iter9（9 次更新后）| 比值 |
|---|---|---|---|
| completion_tokens 中位 | 3,782 | **15,868** | **4.2×** |
| P90 | 4,933 | **50,000** | 10.1× |
| max | **5,843** | 50,000 | — |
| 顶满 50k | 0/30 | **9/30** | — |

**同时证伪了"策略退化"叙事**：匹配后 `p_EXEC` 无差异（base 0.47 vs iter9 0.53，χ²=0.27）——base 本来就有一半概率盲跑 exec。`p_READ` 的下降（0.37→0.07，χ²=7.95）几乎全部来自"想到顶满上限、没发出工具调用"（iter9 的 OTHER 从 0.16 涨到 0.40，其中 0.30 是 `finish_reason=length`）。**策略差异是长度的下游。**

### 一、训练信号的精确形态

```
每个 turn 的 advantage = 本轮 checker ±1 / N
loss 归约 = sum_of_sample_mean（每样本先除以自身 response 长度 T）
loss_mask = 覆盖整段 response，含 <think>
```

**一轮的总梯度贡献 = N × (±1/N) = ±1，与 turn 数、token 数完全无关。**

> 做 6 个 turn × 16,000 token，与做 1 个 turn × 500 token，**对目标函数完全等价**。
...
|---|---|---|
| base | 3,782 | 1/3782 |
| iter9 | 15,868 | **1/15868** |

**同一个 −1，写长 4.2 倍，每 token 只挨 1/4.2 的打。**

而约 **74% 的 file_check 轮次失败**。**在"绝大多数时候在挨打"的环境里，摊薄惩罚就是最有效的自保，而 thinking 是最廉价的摊薄手段**（不需工具调用、不需过 checker）。

**这是梯度下降的选择效应，不是模型"学会偷懒"**：简洁的坏回答，那几个 token 被狠压；啰嗦的坏回答，每个 token 只轻碰一下，行为模式存活得更好。

### 三、本该教"怎么改"的 OPD，覆盖率只有 25%

> ## ⚠️ 本节的"覆盖率"是拿错的尺子量的（2026-09-11 更正）
>
> 用户提出 OPD 的正确语义：**hint 注入这道题开头，要训的是最后产生的答案（比如写好的文件）。**
>
> 按这个语义，"hint 只落在 N 个 turn 里的 1 个"**就是正确行为，不是缺陷**——hint 讲的是最终产物（"文件名不合规"、"时间戳要 ISO 8601"），它本来就只该作用在产出那个产物的地方。我把它除以 turn 数、当成"覆盖不足"，**是在要求一个不该存在的东西**。
>
> **而且逐行核对后：现在的代码已经就是这个语义。**
>
> | 用户要的 | 代码现状 |
> |---|---|
> | hint 注入题目开头 | ✅ `_mc_task_idx` 倒序搜到本轮 task，`_append_hint_to_messages` 贴在那里，后面 transcript 原样接回 |
> | 训最后产生的答案 | ✅ teacher forward 跑在**最后一个真实生成 turn 自己的真实 response** 上 |
>
> **我一度担心 OPD 跑在了 verdict 那条 `max_tokens=0` 的 JSON 请求上（那会让 OPD 近乎空转）——查清了，不是。** `_fire_opd_task(session_id, prev_turn_num, prev_turn_data, messages[-1])`：verdict 请求只作为 `next_state` 去触发**前一个** turn 的 OPD 任务，它自己不成为样本。held 门的条件正是"`next_state` 不是 verdict 就 return"，所以**唯一开火的那个 turn 就是最后一个真实生成的 turn**。
>
> **所以本节"恶性反馈环"（turn 变多 → OPD 占比下降）不成立**——分子分母都不该这么算。下面的 25% 表格保留作记录，但**不要再引用它作为问题依据**。
>
> **本节唯一仍然成立的是最后那句**："hint 贴在最后一个 turn，那时错误早已发生完"——但它现在是一个**可测量的具体问题**而不是结构缺陷：**产出 checker 判定的那个文件的写操作，是不是发生在最后一个真实 turn？** 如果写在更早的 turn、最后一个 turn 只是在说"我写好了"，那 teacher forward 就没落在真正的答案上。**待用 record 实测。**

```python
for _td in turns:                                   # 中间 N-1 个 turn
    await self._submit_rl_turn_sample(_td, reward)  # 纯 RL，无 teacher log-probs
if opd_result.get("accepted"):
    await self._submit_turn_sample(verdict_td, ...) # 只有最后一个 turn 拿 OPD
```

**verdict 只触发一次 `_opd_evaluate`。**所以：

```
OPD 占比 ≈ 失败率 / 平均 turn 数
```

| 平均 turn 数 | OPD 占比 |
|---|---|
| 2 | 37% |
| **3** | **25%** ← 实测 49/194 = 25.3%，吻合 |
| 6 | 12% |

**⚠️ 恶性反馈环**：模型退化 → turn 变多 → OPD 占比下降（分母变大分子不变）→ 纠错更稀 → 纯惩罚占比更高 → 长度压力更大 → 更退化。**day08 那轮 6 个 turn，OPD 覆盖只有 1/6。最需要纠错的一轮，纠错最稀。**

**且位置也不对**：day08 r1 的 t4 撤销了 t3 已经正确的修复，**拿到的是零纠错信息**；hint 贴在最后一个 turn，那时错误早已发生完。
...
| loss 项 | 合成一条后 |
|---|---|
| **RL**（`advantage × logπ`）| **完全等价**。总贡献都是 A；各 turn 等长时每 token 权重也相同（`(A/N)/T_i = A/T_total`）|
| **OPD**（向 teacher 蒸馏）| **不等价**——teacher 前向会覆盖整条轨迹，t1–t4 都能拿到 token 级的纠错 |

**（我最初只算了 RL 项就断言"完全等价"，漏了 OPD，被用户指出后收回。）**

合成一条的**真实好处是 OPD 覆盖**，不是 RL。此外它能抹平轮内不对称（现在同一轮里 500-token 的 turn 被狠压、16k 的 turn 几乎不痛——轮内也在奖励拉长）。

### 五、per-turn OPD 这条路被否掉（用户指出，成立）

我曾提议"每个 turn 各跑一次 teacher 前向，都挂同一个 hint"。**错的。**

**hint 是轮级的**——讲的是"这一轮产出哪里不合格"。贴到 t1（模型在读数据）上，teacher 在"知道文件名错了"的条件下会倾向"别读了直接写"，**蒸馏下去就是教模型跳过必要的中间步骤**。而那个读取恰恰是对的——base 正是靠先读文件才知道列名是 `response_ms`。

### 六、根本约束：**没有 turn 级的 ground truth**

手上只有两样，**都是轮级**：checker 的 ±1、checker 的 hint 文本。

**没有任何东西说明"第 3 个 turn 做得好不好"。**所以任何 turn 级信号都只能是发明的：

| 做法 | 下场 |
|---|---|
| 步骤判官（LLM 开放式判"这步好不好"）| 无事实锚点，**09-03 废弃** |
| per-turn OPD（轮级 hint 评价单 turn）| **归属错误**（上一节）|

**→ 当前把 OPD 挂在最后一个 turn 不是随意的**：那是产出物最终成形处，也是轮级反馈唯一能被正确归属的位置。**它的问题不是错，是窄。**

### 七、由此得到的依赖链

```
想让 OPD 覆盖整条轨迹
  └─ 需要：轨迹成为一个样本（唯一能让轮级 hint 对应轮级评价对象的表示）
       └─ 需要：不剥 reasoning（否则扁平序列声称"t2 看见了完整 t1"，而实际没有
                 —— 2026-09-03 真实训练已证伪，96/120 轮整轮丢弃）
            └─ 需要：先把生成长度压下来
                 （thinking 会在 prompt 里逐轮累积；现在一个 turn 就 16k，
                   按天 10~15 轮直接爆上下文）
```
...
| 步 | 做什么 | 依赖 |
|---|---|---|
| **1** | **改 loss 归约**（`--custom-pg-loss-reducer-function-path`），让拉长有代价 | **无** |
| 2 | 保留 reasoning（不剥）| 1 |
| 3 | 合成一条 | 2 |
| 4 | OPD 覆盖全轨迹 | 3 |

**保留意见**：第 2 步会改变 agent 推理时看到的内容 → **定版基线 41.1%/26.3% 作废需重跑**，且同时偏离 OpenClaw 默认与两篇论文。但它可能连带修掉"t4 撤销 t3 正确修复"这类问题（模型看不见自己为什么那么改）。**是大动作，不能顺手做。**
...
**病根是 2026-09-03 停用步骤判官之后没有填上那个洞。**停用的理由成立（开放式判"这步好不好"、无事实锚点），**但停用后中间 turn 从"有信号（虽不可靠）"变成"零信号"，而 per-turn 的碎片化还留着**。

> ⚠️ **2026-09-11 更正**：原文接着说这是"25% OPD 覆盖率"的来源——**那个覆盖率指标本身是错的尺子**（见第三节横幅：hint 只该作用在最终产物上，落 1 个 turn 就是正确行为）。**"中间 turn 零信号"这个描述仍然准确，但它不是缺陷而是设计**：中间 turn 拿轮级 ±1 作为结果信号，不拿定向信号。真正的问题在梯度稀释（`A/(N·T)`），不在 OPD 覆盖。
...
## 📑 如何阅读本文档（2026-09-07 加，2026-09-18 重整）

本文档 4000+ 行、按时间追加，**其中若干节已被后续证据推翻或下调，均已就地加横幅**。
按结论现状分类如下，避免误引已作废的内容。

### ⛔ 已推翻（不要再引用作为依据）

| 节 | 原主张 | 推翻于 | 实测 |
|---|---|---|---|
| 「🔬 因果分析（09-10）」| `sum_of_sample_mean` 稀释惩罚 ⇒ **模型倾向长回复** | **09-15** | 长度在 **step10 骤跳**（非缓慢爬升），且内容是 **88–96% 字面重复**。至多是循环发生后的**维持**机制 |
| 第三节「OPD 覆盖率只有 25%」| 覆盖率低是缺陷，且构成恶性反馈环 | **09-11** | **量错了东西**：hint 讲的是最终产物，落在 1 个 turn 上**就是正确行为** |
| 「+7.1pt（day01–07）」| 轨迹级样本前半段有效果 | **09-17** | 逐日 **5/7 天 ≤0**，全部来自两个离群日；**Compl 精确为 0** |
| 「Part I 跑法三样都没发布」| 评测 harness / 346 题 / 打分能力都没有 | **09-11** | **两样说重了**。真正缺的只有"基于事实的评分"，题与 checker 都在 `benchmark/` |
| 「准入控制（09-15 上午版）」| MetaClaw 的判据是内容有效/无效/有害 | **09-15 当日** | 判据是**出身**（skill generation 版本），已整节重写为「Support / Query 分离」 |
| 「定版基线 17.8%/0%」| — | — | 作废（session-key 兜底 bug）|
| 「K=6 里程碑」| — | — | 训练增益已被 K=0 证伪 |
| 「剩下的假设…只在 Part I 显现」| — | — | 被自己的对照实验证伪（预测 day 更低，实测更高）|

### ⚠️ 已下调 / 需限定引用

| 节 | 现状 |
|---|---|
| 「方案：轨迹级样本」（09-03 删除）| **删除理由被推翻**（`dropReasoningFromHistory` 是关的），但 **09-17 生产数据显示历史里确实没有思考**（0/127）——**结论对、当时给的理由错**。96/120 轮丢弃的真实成因仍未查明 |
| 「OPD 主导驱动起病」（09-16）| **削弱**：起病前 OPD 确是 RL 的 2–4 倍（09-11 说"弱一个数量级"确实错了），但 `\|diff\|` 全 16 步恒在 0.985–1.000、**跨越 base→85% 复读纹丝不动**，是恒定幅度的扰动而非分级纠错信号 |
| 「计划（09-07）」第二节 `--agent` 分层 | L3 判断出错已更正；据此写的补丁脚本已删除 |
| 查证记录（十二）| 第三节已收窄（"正文反驳了 A.1"是错的）|
| 查证记录（十四）| 成立，但标题已下调（§4.3 贡献有限）|

### ✅ 仍然有效、可直接引用

| | |
|---|---|
| **「🧭 项目定位」** | Part II 方法 + Part I 数据集；目标是专利/论文而非复现 Table 1。**专利已于 09-18 暂时搁置** |
| **「🚪 Support / Query 分离」** | MetaClaw 按 skill generation 版本排除 support data；**我们没有快适应层 ⇒ 训练的全部是 support data**。含 Algorithm 1 原文与 paper↔code 出入 |
| **「📈 放大曲线」** | `0 → 17 → 21 → 27 → 44 → 85`（逐 step 实测），与**全负批期逐点对齐** |
| **「🔑 `grpo_pg_loss` 反解正负构成」** | `(n_neg − n_pos)/8`，已被独立计数验证 |
| **「⛔ 更正（09-17）」** | checker 白送 +1 的实证（day09 r8 零写入得 1.0）|
| 查证记录（十）| 压缩何时触发、补丁语义 |
| 查证记录（十五）（十六）（十七）| Part 归属的最终依据 |
| 查证记录（五）～（九）| 中间步骤信号、GRPO 组、按天 session |

### ❌ 阴性结果（记下来是为了不再重复找）

| | |
|---|---|
| **step10 的触发器不存在于批次里** | 5 正 3 负，正样本 `dup_max` 全 0，无"打转却拿 +1"的样本 |
| **起病处也没有优化器异常** | `grad_norm` 2.59（step9 是 2.43）、`clipfrac`/`ppo_kl` 恒 0、`lr` 恒定 |

**只想知道结论**：看本文档最上面的「🧭 项目定位」与「🔴 当前结论」。
**想知道推理怎么错的**：按上面「已推翻」表的日期顺序读。
**想知道现在跑起来会发生什么**：去 [`training_config.md`](training_config.md)。
**想知道哪些改动真被验证过**：去 [`change_ledger.md`](change_ledger.md)。

### Part I / Part II 归属这条线（09-07 一天内走了六轮）

| 节 | 主张 | 现状 |
|---|---|---|
| 查证记录（十一）| 我们跑的是 Part II 的**题集** | ⛔ **整条撤回** |
| 查证记录（十二）| 公开题集就是 Part I | ⚠️ 第三节已收窄 |
| 查证记录（十三）| §4.1 逐字复核 + 官方配置实证 | ✅ 成立 |
| 查证记录（十四）| checker 实测与 Part I 描述矛盾 | ✅ 成立（标题已下调）|
| 查证记录（十五）| **仓库里是两套独立 agent 栈，我们跑 Part II 那套** | ✅ **成立**，三条独立证据 |
| 查证记录（十六）| 目录归属精确化、train.jsonl 降级、A.3 对称性 | ✅ 成立 |
| 查证记录（十七）| **`train.jsonl` 不是数据集，是一整段真实对话** | ✅ **成立** |
...
> ## ⛔ 本条已于同日（2026-09-07）被硬数据证伪并整条撤回，**不要采信任何结论**
>
> 核心论断"`benchmark/data/metaclaw-bench` 是 Part II 形态的题集"**是错的**。它建立在附录例子的 Part 标签、以及 `\bbox{X,Y}` / `check_iso8601.py` 两处字符串"逐字相同"上；实测证明那两处**是两个 Part 共用的样板**，不构成同一份数据的证据。
>
> **正确结论见下面的查证记录（十二）：这份公开题集就是 Part I，Table 1 的 Part I 那一列对我们是可比的，"4B 打平 GPT-5.2"的异常依然存在、依然未解释。**
>
> 本条全文保留仅为记录当时的推理链条和它是怎么错的。
...
### 查证记录（十二）：公开题集就是 Part I，附录的 Part 标签不可信（2026-09-07）

**本条推翻上面的查证记录（十一），并恢复"异常未解释"的状态。**

#### 起因

（十一）用"我们跑的是 Part II"把"4B 零训练贴平 GPT-5.2"的异常解释掉了。用户提出反向验证：**先确认 Part II 是什么，再倒推 Part I**——因为 Table 1 两个 Part 的数字差异明显不是同一套方法产生的。这个思路把问题从"读标签"变成了"点数据"，于是可判定了。

#### 一、Part II 能确认——确认的方式是它不是我们这份

论文 Part II：14 天 / 588 题 / 每天 42 题 / MC 占 74%。实测公开题集（`benchmark/data/metaclaw-bench/eval/day*/questions.json`）：

```
days: 30   total: 346   MC: 122 (35%)   FC: 224 (65%)
每天题数: 10~15
```

**四项可数特征全部与 Part II 不符，全部与正文 §4.1 的 Part I 定义相符。**

#### 二、附录引的两个"Part II"例子根本不在这份数据里

（十一）判成 Part II 的直接依据是 A.3 两条标着 "Part II, Day 01" 的例子。逐条去数据里找：

| 附录例子 | 公开题集实况 | 结论 |
|---|---|---|
| Part II, **Day 01 / r1** 是 **multi-choice** | day01/r1 是 **file_check**（把 `standup_raw.txt` 整理成 `standup.json`）| 不是同一题 |
| Part II, **Day 01 / r21** → `day01/decision_log_r21.json` | **day01 只有 r1~r10**；全库 `grep decision_log` **0 命中**；**全库最大轮次号 = 15，不存在任何 r21** | 该题不在本数据集内 |

Part II 每天 42 轮，r21 在那边合理。**所以附录那两条确实取自 Part II，而 Part II 的数据官方没有发布。**

（十一）里所谓"逐字相同"实际只有两样：作答格式串 `\bbox{X} or \bbox{X,Y}`，和 checker `check_iso8601.py`。**这两样是两个 Part 共用的 harness 样板，不是同一份数据的证据。**

#### 三、附录的 Part 标签至少错了三处，不能再当证据

> **⚠️ 本小节已于同日按查证记录（十三）收窄。**下表"实际是什么"一列的推断过强：§4.1 全文**没有**描述任何一个 Part 的工具集，因此**正文并未反驳 A.1**；真正与 A.1 冲突的是**公开 benchmark 自己的配置**（`openclaw_cfg/openclaw.json` 用 `profile: coding` 全套工具）。收窄后的正确陈述见（十三）第三节。A.2 那一行也需修正：论文全文从未出现 USER.md / AGENTS.md / TOOLS.md，A.2 只讲 IDENTITY.md 和 SOUL.md。

| 附录条目 | 标签 | 实际是什么 |
|---|---|---|
| A.1（`run_command` 单工具 system prompt） | 标 "Part I" | **RL 训练环境**：`metaclaw/openclaw_env_rollout.py`，仅被 `metaclaw/trainer.py` 引用，**自带文档写明 no `env.evaluate()` / reward=0**，吃 `examples/train.jsonl` |
| A.2（IDENTITY.md / USER.md） | 标 "Part II" | **随 Part I 数据一起发布**：`benchmark/data/metaclaw-bench/workspaces/shared/` 下逐字存在（IDENTITY.md/USER.md/AGENTS.md/SOUL.md）|
| A.3 第三段（`train.jsonl` 真实会话） | 标 "Part I task instruction format" | 同 A.1，是训练数据，不是 346 题 benchmark |

**判定原则改为：只采信正文 §4.1 的定义 + 可数事实（天数/题数/题型配比/轮次号），附录的 Part 标签一律不作为证据。**

#### 四、由此得到的自洽版本

| | 是什么 | 我们有吗 |
|---|---|---|
| **Table 1 Part I** | 公开的 346 题 / 30 天 / 10~15 题每天 | ✅ **就是我们跑的这份** |
| **Table 1 Part II** | 14 天 / 588 题 / MC 为主 / 有 r21+ | ❌ 未发布 |
| A.1 + A.3 的 `run_command` 那套 | RL 训练 rollout 环境，无评分 | 有代码，从未跑过（也不该拿它对 Table 1）|

**这同时结掉了上一轮那个悬案**——"A.1 只有 `run_command`，评测怎么打分"：它压根不是评测器，是 trainer，没有评分逻辑是设计如此。**Table 1 的 Part I 是 `benchmark/scripts/baseline_run.py`（自述 "plain openclaw agent"）跑出来的，跟我们同一条路径。**

#### 五、代价：异常回来了

（十一）的"用对列之后异常消失"**作废**。事实回到：

**同一份 346 题、同一套 checker、同一类 agent 配置，我们的 4B 零训练在 `scope=day` 下 day01–17 打出 Acc 49.9% / Compl 36.3%，论文 GPT-5.2 Baseline 是 41.1% / 14.7%。**

已排除的解释（各自有证据，见前文）：反馈加料、Compl 定义、题目自带白送分（空 workspace 0/224）、会话长度/context 溢出、按题隔离、题集搞错、`--agent` 过度帮助、刷文件数。**目前无成立假设。**

#### 六、（十一）里哪些子结论仍然有效

- "Table 1 只能作量级参照、训练效果一律以我们自己的 K=0 为基准"这条口径**继续有效**——但理由变了：不再是"题集不同"，而是"同题集下我们的绝对分异常偏高、原因未知"。
- `check_filename.py --dir` 不验内容 / 当天累积 / min_count 阶梯——**客观事实仍成立**，但它是 **Part I** 题集自身的设计（原文误写为 Part II）。
- 08-14 起"跟论文 Table 1 对齐"的表述——**恢复为题集确实对齐**；（十一）加的"仅指标定义对齐"限定作废。

---

### 查证记录（十三）：§4.1 / 附录逐字复核 + 官方配置实证，公开题集是"Part I 骨架 + Part II 机制"（2026-09-07）

**收窄（十二）的第三节；不推翻（十二）的核心结论（题集规模 = Part I）。**

#### 起因

用户要求：不要只从数据集入手，去 §4.1 实验建立部分查**两个 Part 的方法差异**；并指出附录呈现的是 Part I = System Prompt + `run_command` Tool Schema、Part II = IDENTITY.md + SOUL.md，追问"**这是全部区别吗，还是有没在附录里表现出来的**"。

#### 零、先纠正一次抓取污染

首次抓取 HTML 全文时，摘要模型返回"§4.1 说 Part I 只有单个 `run_command` 工具"。**复核后证伪**：定向检索 `run_command` 字符串，**全文四次出现全部在附录 A.1，§4.1 不含此词**。摘要模型把 A.1 并进了 §4.1。该句已丢弃。**教训：对全文抓取的转述必须做定向字符串复核，不能直接采信。**

#### 一、§4.1 原文（逐字）

> **Part I** structures evaluation as a 30-workday simulation (346 questions, days 01–30, 10–15 per day). The workspace state (files, configs, project records) persists across rounds within each day, and each question includes the evaluation outcome of the previous round as corrective feedback context. Questions fall into two types: *file-check* tasks … and *multi-choice* tasks … Task difficulty increases monotonically with day index, with days 25–30 requiring sophisticated multi-step reasoning.

> **Part II** extends the evaluation to a 14-workday simulation (588 questions, 42 per day: 434 multi-choice and 154 file-check).

> Part II introduces five implicit preference rules progressively across 14 days. These rules are **not** stated in the agent's system prompt; they must be inferred from task feedback.

> Part I's file-check tasks are **heavily execution-oriented, with many interdependent side effects**, providing a conservative measure of end-to-end completion.
> Part II's file-check tasks are **rule-based transformations** where compliance with behavioral heuristics (e.g., schema conventions, timestamp formats) is the primary bottleneck, making them more amenable to skill distillation.

论文给 Part II 的代号是 **CALMB-14**（见 A.2 原文 "Part II (CALMB-14) injects…"）。

#### 二、§4.1 给出的方法差异，以及**全文都没给的空白**

| | Part I | Part II（CALMB-14）|
|---|---|---|
| 规模 | 30 天 / 346 题 / 10–15 每天 | 14 天 / 588 题 / 42 每天 |
| MC : FC | §4.1 **未给** | 434 : 154（MC 74%）|
| FC 任务性质 | 执行导向、大量互相依赖的副作用 | 规则化转换，瓶颈是遵守行为规范 |
| 隐含偏好规则 | §4.1 **未提** | 5 条 P1–P5 渐进激活，不写进 system prompt，只能从反馈推断 |
| 指标 | 两 Part 相同（accuracy + file-check completion）| 同 |

**附录只覆盖"提示/上下文"一层，且不对称**：A.1 只有 Part I 的 system prompt + `run_command` schema；A.2 只有 Part II 的 IDENTITY.md / SOUL.md 注入；A.7 只有 Part II 的 5 条规则。

**回答用户的问题——附录那两项不是全部区别，而且真正的空白在别处：**

1. **Part II 的 system prompt 和工具集，论文全文从未给出**（附录无 "Part II System Prompt" 条目）→ "Part II 用什么工具"完全未知
2. **Part I 是否也注入 workspace 上下文，从未说明**（A.2 只讲 Part II）
3. **会话粒度（一天一 session vs 一题一 session），两个 Part 都没写**
4. 论文全文**从未出现 USER.md / AGENTS.md / TOOLS.md**

**易误用的一句**：§4.1 的 "All conditions use identical prompts and tool sets" 约束的是 **conditions（Baseline / Skills / Full）**，**不是**两个 Part 之间，不能用它推出两 Part 工具集相同。

#### 三、官方配置实证：公开 benchmark 用的是全套工具，不是单 `run_command`

`benchmark/data/metaclaw-bench/openclaw_cfg/openclaw.json`：
...
**`profile: coding` = OpenClaw 原生全套（read/write/edit/exec/glob/grep），只禁 memory 组；全仓库没有任何地方把工具限制为单个 `run_command`。**

旁证：
- 数据集内 **`TOOLS.md`** 列出 `exec / read / write / exec(ls) / exec(glob) / exec(grep)`，且被 `benchmark/src/check/workspace_integrity.py` 列为**必需文件**（`REQUIRED_WORKSPACE_FILES = ["AGENTS.md","IDENTITY.md","SOUL.md","TOOLS.md","USER.md"]`）
- `run_command` 作为 **agent 工具**在全仓库只出现于 `metaclaw/openclaw_env_rollout.py`；`benchmark/scripts/*.py` 里的 `run_command` 只是 subprocess 的 Python 函数名，无关
- **我们的 driver 用的就是这份官方 `openclaw_cfg/openclaw.json`** → **工具集这一项我们与官方 benchmark 对齐**

**（十二）第三节"正文反驳了 A.1"的说法据此收窄为**：§4.1 根本没描述任何 Part 的工具集，正文未反驳 A.1；**与 A.1 冲突的是公开 benchmark 的配置**。

#### 四、新发现：公开题集同时带着 Part II 的机制

数据集自称 **`"name": "MetaClaw-Evolution-Bench"`**（不叫 Part I）。`all_tests.json` 的 30 条 test 逐天带 `preference_tags` 和 `desc`：

```
day01-05 arc=A  output_format                                          (day01 desc: "P1 first introduction")
day06-10 arc=B  +file_naming                                           P2
day11-15 arc=C  +field_completeness                                    P3
day16-20 arc=D  +workflow                                              P4
day21-25 arc=E  +completion_log                                        P5
day26-30 arc=F  五条全开，混合复现
```

**这正是 §4.1 归给 Part II 的"5 条隐含偏好规则渐进激活"，一条不差**，且确实是隐含的：day01/r1 题面只要求 `meeting_time / attendees / action_items` 三个字段，**ISO 8601 + 时区要求只出现在 `feedback.incorrect` 里**。

加上 A.2 归给 Part II 的 IDENTITY.md / SOUL.md 也在这份数据里且为必需文件。

**结论：公开发布的这份 = Part I 的骨架（30 天 / 346 题 / 10–15 每天，与 §4.1 逐字吻合）＋ Part II 的适应机制（P1–P5 隐含规则 + 身份文件）。它不干净地等于任何一个 Part。**

#### 五、三种可能，暂不定论

| | 说法 | 支持 | 反对 |
|---|---|---|---|
| **A** | 公开这份就是 Part I，论文对 Part I 描述不全（隐含规则/身份注入其实两 Part 都有，只写在 Part II 名下）| 30/346/10–15 逐字吻合；配置就是 coding profile | 那 A.1 的 `run_command` 是什么 |
| **B** | 公开的是作者后来重整的合并版，Table 1 的 Part I 不是这个 artifact 跑的 | 名字是 Evolution-Bench；带 Part II 机制 | 无直接证据 |
| **C** | Part I 评测真的用 A.1 的单工具 agent，公开 harness 是另一套 | A.1 写得很具体 | 公开 harness 无此路径；TOOLS.md + coding profile 反证 |

**若为 C，异常即有解释**（单 `run_command` 靠 shell heredoc 写文件，file-check 完成率远低于原生 `write`，GPT-5.2 的 14.7% 说得通）；**但目前无证据支持 C，且有两条硬证据反对。**

**本条不宣布"异常已解释"。**Part I/II 判断此前已三次改口（详见 work_log 2026-09-07 续三/续四），下一步先读 §4.3 分析部分，看能否从分析口径反推两个 Part 各自用的方法。

---

### 查证记录（十四）：checker 实测证明公开题集与论文对 Part I 的描述矛盾（2026-09-07）

> **本条标题与结论已于同日下调。**初版题为"§4.3 + checker 实测——公开题集用的是 Part II 的方法"，有两处过强，用户追问 "§4.3 具体说了什么，怎么就能判断更偏向 Part II" 后修正：
> 1. **§4.3 几乎没有贡献**，判据实际来自 **§4.1.1**（已定向核验，见下）。初版把 §4.3 抬得过高。
> 2. **"更偏向 Part II"是正面结论，需要"§4.1.1 对 Part I 的描述准确"这个前提**——而本项目刚两次栽在轻信论文对自身组件的描述上（A.1 的 `run_command`、A.2 的 Part 标签）。该正面结论**降级为待定**；**站得住的是否定形式**（见"结论"节）。

#### 判据的真实出处：§4.1.1，不是 §4.3

定向核验结果：
- "Part I's file-check tasks are heavily execution-oriented, with many **interdependent side effects**, providing a conservative measure of end-to-end completion." → **§4.1.1**
- "Part II's file-check tasks are **rule-based transformations** where compliance with behavioral heuristics (e.g., schema conventions, timestamp formats) is the primary bottleneck, making them more amenable to skill distillation." → **§4.1.1**
- 直接字符串检验：§4.1 含 "interdependent side effects" = **YES**；含 "rule-based transformations" = **YES**

**这条判据的价值在于它不是标签而是可量的内容特征**——正是前两轮栽在"读标签"之后所需要的东西。

#### §4.3 实际说了什么（逐字），以及它的有限贡献

小节：Per-day accuracy trends / Task-type breakdown / RL training dynamics / Skill library analysis / Cross-domain skill transfer to AutoResearchClaw / Case studies。**全程不提工具、system prompt、会话粒度**（只在附录 A），故无法裁决工具集问题。
...
**这段完全没有出现 Part I / Part II**（拆的是 MC vs FC）。初版引给 §4.3 的两句 "On Part I, MetaClaw (Skills) leaves…" / "On Part II, where file-check tasks are rule-based…" **无法确认真在 §4.3**（逐字复现的 Task-type 段不含它们，可能来自 §4.2 或系摘要模型改写），**已不作为证据**。

**§4.3 唯一的贡献**：它把 file-check 的成败反复归因于 rule / execution 这条轴（"helps reasoning but not execution"），佐证该轴是作者自己的分析框架，不是我们强加的。

#### 用这个判据实测公开题集：结果与 Part I 的描述完全相反

**1. checker 分布（224 道 file_check）**

```
 70  31.2%  check_filename.py
 65  29.0%  check_metadata.py
 42  18.8%  check_iso8601.py
 40  17.9%  check_backup.py
  7   3.1%  内联 python 一行断言
```

**96.9% 集中在四个通用规则脚本**，全部是单条格式/命名断言。`check_metadata.py` 的 docstring 逐字写着 **"validate P3 metadata completeness"**——**checker 直接按 P1–P5 偏好规则命名**。`check_done_log.py`（P5 completion_log）只在 day21–30（arc E/F）出现，与 P5 的激活时点吻合。

五个 checker ↔ 五条规则一一对应：iso8601→P1 output_format、filename→P2 file_naming、metadata→P3 field_completeness、backup→P4 workflow、done_log→P5 completion_log。

**2. 互相依赖：0 / 224**

脚本化检测"某道 file_check 的 checker 是否检查了本日更早轮次产出的文件"：**0 / 224**。每道题的 checker 只看该题自己要求产出的文件。

#### 比对结论

| §4.1 对 Part I 的描述 | 公开题集实测 |
|---|---|
| "heavily execution-oriented" | ❌ 96.9% 是单条格式/命名断言 |
| "many **interdependent** side effects" | ❌ **0/224 跨轮依赖** |
| "conservative measure of end-to-end completion" | ❌ 判的是规则合规，不是端到端完成 |

| §4.1 对 Part II 的描述 | 公开题集实测 |
|---|---|
| "**rule-based transformations**" | ✅ 四个通用规则脚本 |
| "compliance with **behavioral heuristics** … is the primary bottleneck" | ✅ 反馈里才给规则，题面不给 |
| "e.g. **schema conventions, timestamp formats**" | ✅ `check_metadata`（schema）/ `check_iso8601`（timestamp）|
| 五条隐含偏好规则渐进激活 | ✅ arc A–F，P1→P5 |
| IDENTITY.md / SOUL.md 注入（A.2）| ✅ 且为必需文件 |

#### 结论：区分"只靠测量"和"还需要信任论文"

**（A）只靠测量、不依赖任何信任——确定成立：**

> **公开题集的 file-check 内容，与论文自己在 §4.1.1 对 Part I 的描述直接矛盾**（执行导向 ✗、互相依赖 **0/224** ✗、端到端完成度 ✗）。
...
| 若 | 则 |
|---|---|
| §4.1.1 对 Part I 的描述准确 | 这份数据不是 Part I → **14.7% 不可比** |
| §4.1.1 对 Part I 的描述不准 | 我们不知道 14.7% 是在什么题上测的 → **仍不可比** |

**无论哪条，"4B 打平/超过 GPT-5.2 的 14.7%"这个说法都失去依据。这一步不依赖对论文描述的信任，是本条真正的载荷结论。**

#### 对"异常"的影响：削掉最尖锐的那一形态，但不宣布解决

**成立的部分**：把我们的 Compl 36.3% 去对 Part I 的 14.7%，**从一开始就不是同类比较**——那 14.7% 是在"执行导向 + 互相依赖"的题上测的，而这份公开题集里那种题**一道都没有**。"4B 打平/超过 GPT-5.2"这个说法**失去依据**。

**不能顺势宣布的部分**：这**不**意味着该去对 Part II 那一列。Part II 是 14 天 / 588 题 / 42 每天 / MC 占 74%，我们是 30 天 / 346 题 / MC 占 35%，规模与配比同样不可比。**结论仍是"Table 1 没有任何一格 like-for-like"**，与 09-04 定下的口径（一律以我们自己的 K=0 为基准）一致。

#### 与（十一）（十二）的关系，避免被误读为又转回去

- （十一）说"**我们跑的就是 Part II 的题集**"——**仍然是错的**：Part II 是 14 天 / 588 题，有 r21、有 `decision_log`，公开库里没有。
- （十二）说"**公开题集就是 Part I**"——**只在规模维度成立**，在方法维度被本条否定。
- 本条的载荷结论是**否定式**的：这份数据**不符合论文对 Part I 的描述**。至于"它是不是 Part II 的方法"（（十一）的方向）**仍待定**——见上面（B）。（十一）当初的直觉方向可能对，但它给的证据（`\bbox` 格式串、`check_iso8601` 文件名）是两 Part 共用样板，不足以支撑；本条改用**任务性质 + 依赖结构 + checker 命名**三条独立测量，强度高得多，但**正面归属仍未坐实**。
...
## 计划（2026-09-07）：官方代码路径 + 只修文件落地，重打一次真基线

### 目的

现有两个基线都不能回答"**完全按官方代码库的方法跑，基线是多少**"：

- **17.8% / 0%（定版基线）**：走官方 `metaclaw-bench run`，但 `--agent` 补丁**没打通**（见下），文件落进 `workspace-main/`，`Compl` 恒 0 → 官方方法 + 已知缺陷
- **34.4% / 12.1%（K=0）**：文件落地正确，但走的是**我们的 driver**（按题隔离、自建反馈链） → 我们的方法

**缺的正是中间那一格：官方方法 + 文件落地正确。**这一格拿到之后，才谈得上"我们相对官方代码库偏高了多少"。

### 一、不需要回退训练代码

这条基线走官方 `metaclaw-bench run`，**`metaclaw_rollout_driver.py` 完全不参与**。所以不存在"回退到某个 commit"的问题，只需要三件事：官方 `infer_cmd.py` + `--agent` 补丁、OpenClaw 系统补丁 2–6、`rl-training-headers` 关闭。

### 二、`--agent` 是一条**四处**断链，缺任一处整条静默失效

> **⚠ 更正（2026-09-07）**：本节初版写成"L3（`_run_group` → `_run_question`）上游本来就是对的"，**是错的**。我把 `infer_cmd.py:915` 的 `agent_id=agent_id` 当成了那个调用，实际它属于**同一函数内的 `_execute_update(...)`**——按行号匹配而没看它属于哪个调用。据此写的补丁脚本 `prepare_patched_metaclaw_baseline_agentfix.sh` 只补 L1/L2/L4，**落到干净上游会因 L3 断链而完全无效**（正是它自己警告的那个失效模式）。该脚本已删除，改为下面的"只校验不打补丁"。

| 层 | 位置 | 干净上游状态 |
|---|---|---|
| L1 | `_run_openclaw_agent` 的 argv | **没有 `--agent`** ❌ |
| L2 | `_run_question` → `_run_openclaw_agent` | 不传 `agent_id` ❌ |
| L3 | `_run_group` → `_run_question` | **不传 `agent_id`** ❌（初版误判为 ✅）|
| L4 | `_run_group` → `_run_openclaw_agent`（末轮 standalone feedback）| 不传 `agent_id` ❌ |

**缺任何一处，`agent_id` 一路默认成 `None`，argv 里的 `--agent` 消失，行为与完全没修逐字节相同。**这就是"号称打了 agentfix 却仍 `Compl=0`"最可能的成因。

### 二之二、服务器上已有完整修复，不需要打补丁

2026-09-07 查看 `MetaClaw-official` 工作区（`git status` 显示 `M benchmark/src/infer/infer_cmd.py`），diff 内容为**四处齐全的正确修复**（L1 用 `cmd` 列表 + `if agent_id: cmd.extend(["--agent", agent_id])`，L2/L3/L4 各补 `agent_id=agent_id`），**比本项目原先设计的补丁更完整**。另含一处独立改动：

- `_wait_for_gateway` 的 `timeout` **10.0 → 30.0**。这**不是** `--agent` 的一部分，是等网关启动的容错。**决定保留**——它不触及任何评分逻辑，去掉反而会在服务器负载高时引入假失败；但必须记进 manifest，"完全按官方代码库"这个表述要相应打折。

注：该 checkout 是浅克隆（`922caf3 (grafted)`），历史被截断，**查不到这处修复是何时引入的**，因此无法判定定版基线 17.8%/0% 当时是否已有完整修复。不再推测。

### 二之三、改为"只校验不打补丁"

启动脚本 `scripts/metaclaw/run_official_baseline_modelfactory.sh` 内置校验器，**四处缺一即 FATAL 退出**，不自动补——自动补会在上游变动后悄悄补歪，而"悄悄补歪"正是这次要消除的失效模式。

校验器按 **caller + callee 配对**判断，不看行号，因此不会被同函数内的 `_execute_update(agent_id=agent_id, ...)` 迷惑（即上面那个把我带偏的坑）。

**本地验证（三个方向全过）**：
- 干净上游 → FATAL，四处全部列出，exit 1
- 复现服务器版（四处齐全）→ `ok`，exit 0
- **单点回归：仅移除 L3** → FATAL 且**精确只指出 L3**，exit 1

### 三、压缩 / 50000 —— 用户提出的风险点

**配置事实**（`benchmark/data/metaclaw-bench/openclaw_cfg/openclaw.json`）：

```
contextWindow: 50000    maxTokens: 50000    agents.defaults.compaction.mode: "safeguard"
```

`reserveTokens` 未覆盖，取 OpenClaw 默认下限 **16384** → 压缩应在 transcript 逼近 `50000 − 16384 ≈ 33.6k` 时触发，**距硬墙还有约 16k 缓冲**，正常情况下不会"压缩之前就报错"。

**那一次的实测**：seed 465485731 的 context overflow = **4.6%**（另两个 seed 17.9% / 5.5%）。且 **cli-compaction 补丁当时就在**（补丁 2–6 含 `cli-compaction`），即"压缩失败 → 抛错杀回合"已被降级为 skip-and-continue。**所以那一次绝大多数轮次是压缩正常工作、不是压缩前报错。**

**但这不能直接外推到修复后的这一次**：文件落地正确 → checker 通过 → 反馈文案由 incorrect 变 correct，且 agent 可能回读文件核对，token 走向不可预测（两个方向都有可能）。**必须实测，不靠推断。**

**前置冒烟（必做，先于全量）**：单独跑 **day30**（15 轮，全库最长的一天，且难度随 day 单调上升 → context 最坏情况）。day30 零 overflow 则全量安全。

**三个计数（冒烟和全量都要抓）**：

```bash
grep -c "openclaw-rl-cli-compaction-patch" <log>   # 压缩失败被跳过的次数
grep -ci "context overflow\|context_length\|exceeds"  <log>
grep -c "Exit code\|Timeout after"                 <log>   # infra 失败
```

判读：三个数都小 → 结果可直接归因；任一显著 → 该次 `Compl` 混了压缩/溢出效应，**只能当上界**。

### 四、`-w` / `-n`：采用官方默认，理由已查证

官方 `benchmark/scripts/baseline_run.py` 的默认是 `BENCH_WORKERS = 15`、`BENCH_COUNT = 3`；我们定版基线用的是 `-w 1 -n 0`。

- **`-n 3` 采用**。查过 `_run_question` 的重试逻辑：`for attempt in range(retry+1): rc,… = await _run_openclaw_agent(…); if rc == 0: break`——**只在 `rc != 0`（进程失败/超时）时重试，不会拿错答案重摇**。所以它不抬分，反而减少"infra 失败被记 0"的污染，既忠实于代码库又更干净。
- **`-w` 与分数无关**（每个 test 独立 work copy + 独立 gateway + 独立 session），只影响墙钟和超时风险。15 个并发 OpenClaw 打同一个 SGLang（每个 50k 上下文）容易触发超时，**建议按服务器实测能力定，先取 4**；确认吞吐充裕再往上加。**`-w` 取值需记进 manifest**，否则后续对照说不清。

### 五、必须保持不动的

- `rl-training-headers` **关闭**
- OpenClaw 系统补丁 **2–6 全开**（`sglang` / `embedded-agent` / `system-prompt` / `cli-compaction` / `silent-reply`）——与定版基线同状态
- **我们后来的 metaclaw 补丁一律不进**（round-group、opd-hint、step-judge、verdict-* 等全部不加载）
- SGLang seed 固定 **465485731**，与 17.8% 那次直接可比

### 六、预期判读

| 结果形态 | 含义 |
|---|---|
| Acc ≈ 17–18%，Compl 由 0 变非 0 | `Compl=0` 纯是文件落地问题，Acc 不受影响——最干净的情形 |
| Acc 同步大涨 | 落地修复连带改变了反馈链，影响面比预期广，需重新评估此前所有 Acc 结论 |
| overflow / 压缩计数显著升高 | 落地正确让会话变长，该次只能当上界，需考虑调 `reserveTokens` 或分析压缩时机 |

**额外价值**：这一格与我们 driver 的 `scope=day` K=0（day01–17 为 49.9% / 36.3%）是**同为按天会话**的两个数，差值直接量化"我们的 driver vs 官方 harness"的结构差——这是目前对那个异常的第二个独立抓手。
...
> **⛔ 本小节的假设已被它自己的对照实验证伪（2026-09-07）。** 假设预测"按天共用会有毒历史、分数应更低"，实测 `scope=day` 的 day01–17 是 **49.9% / 36.3%**，反而**高于**按题隔离的 41.4% / 21.0%——方向相反。下面这套 Part I/II 不对称的解释**不再成立**，Kimi 2.0% 是"塌陷不是能力下限"的推论同样作废。**开关本身有效并已用于对照，保留；只有这段动机作废。**当前假设清单见查证记录（十二）。
