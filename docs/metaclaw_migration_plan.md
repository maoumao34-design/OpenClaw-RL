...
## CLI 诊断：`094611`（全量 30 天）里"超长 thinking 空转"复现的真实机制——不是 Acc→0 本身，是中段 step-judge 长期压过最终 checker

**背景**：`455a54f` 那三处修复（追加真实反馈、`FORMAT_ERROR` 去重、`is_invalid_tool_use` 接线）针对的是 day12-14 的表面症状；这次 CLI 对同一批（`metaclaw_migration_20260820_100115`）日志做了更深的时间线分析，结论是：**Acc.=0 和 thinking 爆炸本身都是结果/表象，不是根因**。

**先划掉的表象**（都不该当根因）：
- `day15+ Acc=0`——`day6`/`day9` 早就出现过整天 `Acc=0`，`day15` 只是变成连续全灭，不是新现象。
- `thinking 到 10 万+/finish_reason=length`——从 `day17` 才主导，是策略已经坏了之后才出现的形态，不是诱因。
- `idle timeout 文案/FORMAT_ERROR 里夹超时`——`day17` 起 agent 几乎只吐这句，是生成已经死掉的外显。
- `degraded-turn-drop`/`is_duplicate_user_retry`——`day17+` 训练样本变成 0 的锁死机制，会放大崩溃，但不是最早点火的东西。

**真实时间线，三个阶段**：
1. **阶段 A（约 day1-10）：奖励从一开始就教错了"什么叫好"**——最终失败的 round 里，中段 step-judge 仍有 71% 是 +1；57/83 道最终失败题的**全部**中段步骤都是 +1。到 `day7-10`，任务 Acc 已经很差，但中段 RL+ 仍约 90-100%。也就是说 batch 里大量样本是"过程像在做事"（写文件、列步骤、看起来在推进）拿到 +1，最终 file_check 的 -1 被数量淹没——**学到的是"过程表演"，不是"checker 真的通过"**。`day6` 起 P2 命名规范上线、Acc 可以整天为 0，但中段仍在大量 +1——**能力不够只是触发器，错位的奖励结构才是训练动力学层面的根因**。
2. **阶段 B（day11-16）：恶化，正信号塌、工具开始空转**——中段 RL+ 比例从 `day10` 约 91% 一路掉到 `day14` 41%、`day15` 15%；`day14-15` 的 `invalid-tool` 罚分暴涨（13→31），且多数是"低复读次数 + 反复乱调工具"，不是规则 5 那种 `copies≥12` 逐字复读。这个阶段模型仍在尝试调工具，但已经在空转；中段奖励从"乱赏 +1"变成"乱打 -1"，策略两头被拉扯。
3. **阶段 C（day17-22）：锁死——生成死亡 + 学不到东西**——几乎只剩 `finish_reason=length`、`tool_calls=0`，`max_sentence_copies` p90 到 130+。对外输出变成 idle timeout 文案，但因为 `agent_succeeded=True`（rc=0，只是内容是这段兜底文案），仍按 0 分正常进 Acc.，超时文案还会进下一题的 `[Previous Feedback]`。训练侧：这个阶段几乎全部样本被 `is_duplicate_user_retry` 判定成 `degraded-turn-drop`，OPD/RL 提交量变成 0——**最需要纠偏的时候，梯度反而断了，形成自锁**。

**根因结论**：
- **主根因**：中段 step-judge 打分**跟最终任务成败解耦**——本质是信用分配错误。在最终会失败的轨迹上系统性地给中段 +1，等于用 GRPO 在强化"看起来在推进"，而最终 -1/OPD hint 的信号权重相对太稀，长期训练下会被稀释掉。持续全量训练时，P2→P3/P4 难度阶梯只是把这条错误信号暴露、放大出来——**K=6 冻结实验因为只训了 6 天就停，没有跑到"暴露"这一步，不代表这条信号本身没问题**，这也是为什么这条根因值得单独记录、不能因为 K=6 结果好看就忽略。
- **次级根因（崩溃为什么不可逆）**：生成一旦掉进超长 thinking/idle timeout，`is_duplicate_user_retry` 会把这整段残局样本丢掉——没有 -1、也没有 OPD hint 能把它拉回来。这是"崩溃为什么锁死"的机制，不是最初教错的那个动作。
- **`455a54f` 为什么"有效但不够"**：那三处修复动的是反馈文案/`is_invalid_tool_use` 接线（对应阶段 B/C 的表面症状），没有触碰"失败题中段仍大量 +1"这条阶段 A 就存在的主根因，所以能推迟全灭发生的时间点，但挡不住这整条链路最终发生。

**pause-retry 扩展（本文档上一节）跟这条链路无关**：那次修复只影响暂停期间被误杀的计分（infra failure 层面），完全不碰奖励结构本身。

**CLI 给出的对症方向（未讨论定稿，记录供下一步参考）**：①重做中段奖励——失败最终局的中段不能默认 +1（至少：没有有效 write/edit、或者最终必败时，中段不进 +1/要降权）；②保证最终成功信号（checker ±1 + OPD）在 batch 里的权重不被中段样本淹没；③给"锁死"留一个出口——超长 thinking/idle timeout 不能只靠 drop，要么生成侧截断，要么至少留一个可学的 -1，不然 `day17` 这个模式会复现。这三条都还没跟用户/设计讨论定稿，不是这次要实现的方案。

**跟 MetaClaw 官方设计的关系（用户追问，已用真实源码核实）**：直接读了 `metaclaw/prm_scorer.py`。MetaClaw 自己对**每一轮**（不区分中间/最终）都用同一个 `PRMScorer` 打分，判官 prompt 原文明确写"Do NOT compare against any follow-up turn. Only evaluate whether the response addresses the given instruction."——是刻意设计成纯局部、逐轮独立的判断，从不看这一步之后发生了什么、更不看整条任务最后有没有成功。MetaClaw 自己的训练奖励（不管中间还是最终）**从来没有任何确定性/最终结果信号参与**——他们的确定性 checker 只用来离线算 Table 1，从不进 RL 训练循环。**这意味着"中段奖励跟最终结果脱钩"这个问题在 MetaClaw 自己的架构里根本不会显形**——不是他们解决了这个问题，是他们的设计从没让确定性最终结果参与过训练奖励，所以这个矛盾在他们那边不存在。**这次迁移是我们自己主动引入"最终用 checker 确定性 ±1"这个设计**（比 MetaClaw 纯主观判官更贴近真实结果，这次迁移一开始就做的刻意提升），但也正因为这样，才第一次制造出"中间步骤判官（仿 MetaClaw 判官 prompt 写的）可能跟最终 checker 结果互相矛盾"这个 MetaClaw 原始设计里根本不存在的新问题——**不能指望照抄 MetaClaw 的处理方式，因为对方架构里没有等价物**。唯一有参照价值的是 `toolcall-rl`（OpenClaw-RL 自己的 General Agent 赛道，"三方对照"一节已记录）：整条轨迹只算一个 Sample，中间轮次跟最终轮次通过 `loss_mask` 共享同一个 outcome reward，天然不会出现"中间步骤单独乱打分"的问题，但代价是要放弃真实 `openclaw agent` CLI 子进程（当初为保真度被排除）——是设计新方案时值得回看的参照点，不是能直接照搬的答案。
...
**跟 CLI 之前诊断的"中段 step-judge 正反馈压过最终负反馈、信用分配错误"是什么关系**：这是一个**独立但可能相互放大的新根因**，不是同一个问题的另一种说法——CLI 那次诊断的是"中间步骤的判官打分"跟"最终结果"脱钩；这次发现的是"最终结果"本身（至少对这一类 file_check 题）就不是这一轮真实表现的可靠反映。两者叠加起来，训练信号的可信度问题比之前认识到的更严重：不只是"中间步骤可能教错"，是"连当作事实依据的最终 checker 分数，在这类题上都可能是错的"。这条**需要用真实数据核实**——具体多少比例的 file_check 失败题属于这种"这轮自己写对了、但因为前面题目的历史欠账被判 FAIL"的情况，值得让 CLI 去查（对照 checker 真实 stdout 里 `found M`/`expected >= N` 这两个数字和这一轮 workspace 里实际新增的文件，能不能推断出"这一轮自己有没有写对"和"这一轮判分"是否一致）。
