...
**不另外跑 2 天冒烟，直接按全量 30 天跑。** 理由成立：原本设想的四项冒烟检查全部可以从**同一次全量跑的 day01~day02 日志里读出来**，冒烟run 是全量run 的真子集，单独跑一次只是重复一遍环境搭建。

所以改成**在全量跑内部设一个早期中止检查点**——day02 跑完时看下面四条，任何一条明显不对就当场杀掉，不要白烧 30 天：

| 检查点（day01~day02 日志） | 期望 | 不达标怎么办 |
|---|---|---|
| ① 前缀重建成功率 | 接近 100% | **明显失败就立刻中止**，方案不成立，回来重新设计观测段的取法 |
| ② 每天提交的样本数 | 约等于当天 round 数（day01=10、day02=11）| 少很多说明重建在悄悄丢数据，中止排查 |
| ③ 轨迹 token 长度分布 / 硬丢弃命中率 | 硬丢弃只命中少数失控 round | 命中率高说明 24576 这个阈值或轮数上限要先调 |
| ④ OPD hint 接受率 | 不显著低于现在 | 掉得多说明 round 级 hint 判官不认，需要单独处理 |

四条都过就让它跑完 30 天，**这一次要看分数**——跟"跑到 day22"那次（`20260827_163030`）和"只训到 day6"那次比 Acc./Compl. 曲线。

**关于 `n_samples=1` 的局限，仍然照上面所说成立**（43 次更新、约 23% 的批次全负、advantage 就是原始 ±1），所以：**跑出来不好不能直接判方法死刑**，因为 `n_samples=1` 本来就缺组内比较；届时的下一步是上 `n_samples=8` + 打开 `rewards_normalization`，而不是推翻轨迹级这个形态。反过来，**如果 `n_samples=1` 就已经比 day22 那次好，那 `n_samples=8` 只会更好**，可以直接接着做。
