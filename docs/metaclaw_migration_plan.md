...
1. **代理侧硬丢弃**：拼完的轨迹 token 数超过阈值 → 打日志 + 丢弃整轮，不提交。永远安全，无需新机制。**阈值必须从 `--max-tokens-per-gpu` 反推**：开着 `--use-dynamic-batch-size` 时 slime 按 token 预算把样本打包成 micro-batch，**单条样本长过这个预算就永远打不进任何一包**，所以 32768 是硬天花板，不是建议值。
...
**结论修正为：batch 8 的更新次数约为 day22 那次的一半到七成（按天数折算）。**这不是数量级差距，但**读结果时必须记住这一条——即使形态是对的，分数也可能因为更新次数少而偏低**。为什么仍选 batch 8 而不是 4：`n_samples=1` 时 batch 4 的全负批概率是 `0.83^4 ≈ 47%`，batch 8 是 23%，batch 16 是 5% 但只剩 21 次更新。**batch 8 是这三者里最不坏的折中，不是最优解**；等上了 `n_samples=8`（全负组会被常数桶逻辑丢弃、不再是均匀打压），batch 4 就变得可选了。
...
2. **即使把阈值顶到硬天花板 32768，全程仍有约 13% 的轨迹进不了训练**；卡在 24576 是约 17%。两者差别不大，**说明问题不在阈值选多少，而在于长轨迹本身太多**。
3. **被丢掉的是系统性偏向 day08+ 和失控轮的长难 round**——这是真实的选择偏差，不是随机丢弃。**所以轮数上限从"可后置的优化"升级成"缓解偏差的主要手段"**：与其在拼完之后丢掉一条 47k 的轨迹，不如一开始就不让 round 跑到那么长。
...
改成**在全量跑内部设两个早期中止检查点**。设两个而不是一个，是 CLI 的 ⑥ 直接逼出来的：**day01–02 的轨迹又短又少（约 2.0 samples/round、>24576 命中率约 7%），根本代表不了 day08 之后的情况（约 11.7 samples/round、全程命中率约 17%）**——只看 day02 会得到一个过于乐观的结论。
...
| ④ 轨迹长度 / 硬丢弃命中率 | 按 CLI 数据，此处应在 10% 以内 | 这里就超了 = 后面只会更糟，中止 |

**检查点 B（day08–day10 跑完）——验长轨迹这一段扛不扛得住：**

| 项 | 期望 | 不达标怎么办 |
|---|---|---|
| 硬丢弃累计命中率 | 不超过约 20%（CLI 全程基线是 17%）| 明显更高就中止，先上轮数上限再重跑 |
...
- **跑得不好不能直接判轨迹级形态死刑。**`n_samples=1` 本来就没有组内比较（约 23% 的批次全负、advantage 就是原始 ±1），更新次数也只有 day22 那次的一半到七成。那种情况下的下一步是**上 `n_samples=8` + 打开 `rewards_normalization`**，而不是推翻这个形态。CLI 的说法很准：`n_samples=1` 下全负批问题只是**从"186 个 -1"变成"8 个 -1"，没有消失**。
