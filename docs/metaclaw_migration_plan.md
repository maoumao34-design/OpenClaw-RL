...
## 里程碑：K=6 冻结实验——首次观测到训练带来的真实、可信的能力提升（2026-08-20c）

**这是本次迁移第一个可信的"训练确实有用"的正面结果**，用的正是当天早些时候实现的 `METACLAW_TRAIN_UNTIL_DAY` 功能（`commit 0882f69`）——训练 day1-6、day7 起冻结权重继续跑完剩余天数，把"边训边考的滚动数字"和"能不能真的评估训练效果"这两件事第一次干净地分开。

**关键前提，必须记清楚**：**这次实验跑的是 `commit 0882f69`，不包含同一天稍晚才诊断/实现的"day12-14 超长 thinking 空转"三处修复（`commit 455a54f`）**——那三处修复是针对另一次全量 30 天训练（`094611`）的诊断结果，这次 K=6 实验在设计和实现上都早于那次诊断。下面这次实验里"冻结窗没有出现 094611 那种后期 Acc→0 塌陷"，**不能理解成"thinking 空转问题已经被验证解决了"**——这次没有崩，结构性原因是训练在 day6 就冻结了，day7 之后模型不再继续在（可能已经退化的）训练信号上更新权重，天然绕开了"继续训练→继续吃刻板反馈→陷入空转"这条链条，跟 455a54f 那三处"修反馈内容/接通已有复读检测"的修复是两件不同的事——那三处修复要在一次**继续训练到 day12 之后**的跑里才谈得上验证。

**设置**：
- `commit 0882f69`，`METACLAW_TRAIN_UNTIL_DAY=6`
- 训练起点：干净 base Qwen3-4B（不是 Personal Agent Track 训完的 checkpoint）
- 日志目录：`logs/metaclaw_migration_20260820_122808`，报告：`.../122808/report/report.md`/`report.json`

**主结果**：

| 窗口 | 含义 | Acc. | Compl. | n |
|------|------|:---:|:---:|:---:|
| Train day1-6 | 边训边测（滚动权重，只作过程监控，不是评测结论） | 61.4% | 45.0% | 63 |
| Frozen day7-30 | day6 后权重冻结，真正回答"训练有没有用" | **31.9%** | **7.1%** | 280 |
| 全跑混合 day1-30 | 训练/冻结数据混在一起，不宜单独解读 | 37.3% | 13.9% | 343/346 |

（训练窗有 3 题未进分，故 343 而非 346；Train day1-6 各天 Acc.：40/53/71/53/83/63。）

**相对新基线（seed 465485731，Acc.=17.8%/Compl.=0.0%）的对比**：基线里跟 Frozen 窗口同范围的 day7-30 子集 Acc. 约 16.2%——**Frozen 窗口 31.9% 相当于 +16 个百分点，`Compl.` 从 0% 到 7.1%，是这次迁移第一次观测到非零的 `Compl.`**。这个对比是干净的：基线和这次实验用的是同一批 08-20 修复（`--agent`/session 拆分/训练信号/OPD hint 接线等六处），不存在"新基线本身也有 bug、対比不公平"这种顾虑。

**冻结窗逐日 Acc.**：day07 21.2 → 08 37.9 → 09 49.4 → 10 46.4 → 11 40.0 → 12 31.5 → 13 37.5 → 14 25.8 → 15 22.3 → 16 46.7 → 17 27.0 → 18 36.1 → 19 35.3 → 20 35.6 → 21 36.7 → 22 29.5 → 23 21.5 → 24 25.8 → 25 20.5 → 26 33.3 → 27 36.4 → 28 30.8 → 29 22.0 → 30 22.9——逐日数字有正常波动，但没有 `094611` 那种"day12 起持续塌到接近 0"的模式。

**如实记录的局限（不夸大这次结果）**：
- Train 窗口 61.4%/45.0% 是边训边考的滚动数字，前提是权重还在更新，**不能当成最终评测结论**，只作过程监控参考。
- 训练路径（走 proxy/`rl-training-headers`）和官方 `metaclaw-bench` 独立评测路径不完全同构，这次冻结窗口的评测走的仍是训练自己的 harness，不是重新拉一次官方 `metaclaw-bench run` 单独测这个 checkpoint——数量级对比站得住，但严格意义上的 apples-to-apples 对比，应该再单独用官方 bench 跑一次这个冻结后的 checkpoint 核实。
...
**意义**：这是这次 MetaClaw 迁移从"改 bug、修链路"阶段第一次拿到的正面训练效果证据——用 Train/Frozen 窗口拆分这个方法学本身也第一次在真实数据上验证了可行性（跟 CLI 最初讨论这个方案时"这样能不能干净地回答训练有没有用"的设计目的完全对上）。
