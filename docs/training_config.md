...
> ## ⚠️ 维护规则
>
> **改了训练配置，必须同时改这里，并让 `scripts/tests/test_training_config_doc.py` 通过。**
> 该测试把本文档里的关键数值逐条对着真实脚本核，**文档写错或脚本改了没同步，测试会挂**。
> 这不是形式主义：本项目已经因为"注释停在旧版"和"补丁打错层"各栽过一次。

**最后核对：2026-09-08，commit `e7979e1`**
