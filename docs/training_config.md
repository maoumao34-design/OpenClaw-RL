...
**快照对应：2026-09-21**（轨迹级样本 + 超长轮次熔断 + 熵系数可调；补记 `m_effective=1` 与 advantage 无 baseline）（此后若有未同步的改动，以代码为准）
...
| **候选数 `m_effective`** | **1**（≠ 论文 m=3）。MetaClaw verdict 分支返回单候选，`--hint-m` / `--hint-selection` 是死参数，`sel_k_star_mean` 恒 0.0。**这是迁移设计后果，不是缺陷**——见 [`WARNINGS.md`](WARNINGS.md) |

> ⚠️ **`advantage = reward` 意味着没有 baseline，拿到 +1 的样本被无条件推高。**
> 组大小恒为 1（见下节 `--n-samples-per-prompt 1`），**「组内对比」在本配置下不存在**
> ——2026-09-20 曾在这个不存在的前提上推演过一整轮分析，**引用前先回到这一行**。
...
| `--kl-loss-coef` | **默认 0.0（官方值），可由 `METACLAW_KL_LOSS_COEF` 覆盖** | 2026-09-21 加覆盖口。官方脚本本来就带 `--use-kl-loss`、`--ref-load` 指向 `Qwen3-4B-Thinking-2507` 初始权重，`openclaw_topk_select_loss.py:452-460` 也已实现 KL 项——**只差这个系数，不需要改 loss 代码**。`placement_group.py:182` 的 `with_ref` 早已为真，**ref 模型现有每一趟都在加载，开 KL 不新增显存** |
| `--kl-loss-type` | **`low_var_kl`** | 官方脚本**显式设定**，未改。⚠️ **不是 slime argparse 的默认值 `k1`**——2026-09-21 曾按 argparse default 误记为 k1 并据此提了一条"估计量可能为负、会鼓励偏离"的风险，**该风险不存在**：`low_var_kl` 是低方差且**恒非负**的估计量。查这类参数要读脚本实际那一行，不是读 argparse |
| `--ref-update-interval` | **None（不刷新）** | slime 默认，未设。**设了它 ref 会被周期性刷成当前 policy，"锚到初始模型"的语义就没了**；profile 里有断言拦截 |
...
| **没有任何正则把策略拉住** | `--entropy-coef 0.00` **且** `--kl-loss-coef 0.0` —— 既不拉向高熵也不拉向 base，跑的是纯奖励最大化。**2026-09-18 三臂消融**：零训练基线跑完 30 天且 `DROPPED=0`，而纯 RL / 纯 OPD / 混合**三个训练臂全部退化成复读、无一跑完**。复读按定义即低熵行为。熵系数 2026-09-18 已可调、**KL 系数 2026-09-21 已可调**（均见第七节）。⚠️ **熵那条路已证伪**：系数 0.1 确实把熵抬了 +305%（0.157→0.638）而崩得更早更狠——token 级熵与序列级复读解耦，熵是观测错了量 |
