...
**完成内容（续，同一天）——里程碑：K=6 冻结实验，第一次拿到可信的正面训练效果证据：**
...
- **关键前提**：这次实验用的是 `0882f69`，**不包含同一天稍晚才诊断/实现的"day12-14 thinking 空转"三处修复（`455a54f`）**——那三处修复针对的是另一次全量训练（`094611`）的诊断，这次实验在设计上早于那次诊断。冻结窗口这次没有出现 `094611` 那种后期 Acc→0 塌陷，**结构性原因是 day6 后不再继续训练**，天然绕开了"继续训练→继续吃刻板反馈→陷入空转"这条链条，不代表 thinking 空转问题已经被验证解决——要验证那三处修复，需要另跑一次带上 `455a54f`、且训练继续到 day6 以后的实验
- **主结果**：Train day1-6（滚动权重，仅过程监控）Acc.=61.4%/Compl.=45.0%（n=63）；**Frozen day7-30（真正的评测数字）Acc.=31.9%/Compl.=7.1%（n=280）**；全跑混合 37.3%/13.9%（n=343/346，不宜单独解读）
- **相对新基线（seed465485731，Acc.=17.8%/Compl.=0.0%；同范围 day7-30 子集 Acc. 约 16.2%）**：Frozen 窗口 Acc. 31.9% 相当于 **+16 个百分点**，`Compl.` 从 0% 到 **7.1%**——这是本次迁移**第一次观测到非零的 `Compl.`**，且基线和这次实验用的是同一批 08-20 修复，对比是干净的，不存在"基线本身有 bug"这种顾虑
- 冻结窗逐日 Acc. 有正常波动（21.2~49.4 之间），没有 `094611` 那种持续塌陷模式
- **局限如实记录**：Train 窗口数字是滚动权重下的过程监控，不是评测结论；这次冻结窗口的评测走的仍是训练自己的 harness（proxy/`rl-training-headers`），跟官方独立 `metaclaw-bench run` 不完全同构，量级对比站得住，严格 apples-to-apples 还需要另外单独用官方 bench 重跑一次这个冻结 checkpoint
