...
2. **MetaClaw 迁移**：**轨迹级样本方案已写完，等 CLI 在真实环境查证九点**（第 1 条前缀重建成功率不过则整个方案不成立）。通过后**直接跑满 30 天、不单独做冒烟**（四项冒烟检查从同一次跑的 day01~day02 日志里读，day02 跑完设早期中止检查点，不达标当场杀掉）。见迁移文档"方案：轨迹级样本，一个 round = 一个样本"。**代码仍停在干净基线上，方案未实现。**
3. 其余同 08-17
