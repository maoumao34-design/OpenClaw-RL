...
**完成内容（续，同一天）——两项重大发现，均只记录、未讨论定稿修复方案：**
- **CLI 深挖 `094611`（全量 30 天）时间线，定位"超长 thinking 空转"复现的真实机制——不是 Acc=0/thinking 爆炸本身，是中段 step-judge 长期跟最终结果脱钩**。三阶段：阶段 A（day1-10）最终失败题里 71% 的中段步骤仍是 +1（57/83 道全部中段步都是 +1）——batch 里"过程像在做事"的正反馈把最终 checker -1 的信号淹没，学到的是"过程表演"不是"真正通过"；阶段 B（day11-16）中段 RL+ 比例从 91%→15%，工具开始反复乱调；阶段 C（day17-22）几乎只剩 `finish_reason=length`+`idle timeout` 文案，且这类样本被 `is_duplicate_user_retry` 整段 drop，训练样本变 0——最需要纠偏时梯度反而断了，锁死。**主根因是中段奖励结构性教错，`455a54f` 那三处只动了阶段 B/C 的表面症状，没碰这条阶段 A 就存在的主根因**，能推迟但挡不住
  - 用户追问 MetaClaw 官方对中间步骤是不是完全不管，核实（读 `metaclaw/prm_scorer.py` 源码）：**不是不管，是每一轮（不分中间/最终）都用同一个判官打分，且判官 prompt 明确写"不看后续轮次"**——MetaClaw 自己的训练奖励从来没有确定性最终结果参与，所以"中段跟最终脱钩"这个问题在他们的架构里根本不会显形；这次迁移是自己主动引入"最终用 checker 确定性 ±1"这个设计才第一次制造出这个矛盾，MetaClaw 没有对应解法可抄。`toolcall-rl`（loss_mask 共享 outcome reward）是唯一有参照价值但要放弃真实 CLI 的方案
→ 详见 `metaclaw_migration_plan.md`"CLI 诊断：`094611`...超长 thinking 空转复现的真实机制"
...
- **发现（用户，已用真实代码+真实题面核实）：`check_filename.py --dir --min-count` 是累计跨轮次计数，一旦落后就结构性补不回来——正确的做法也会被判负分**。读了 `check_filename.py` 全文和真实 `day06/questions.json` 的 `eval.command` 序列（`r1` 默认 min-count=1、`r2` 改成 2、`r4`→3、`r7`→4、`r9`→5，逐题递增），确认：checker 每次现场数当天共享目录里有多少个合规文件，跟 `min_count` 比大小，不是只看这一轮新写的文件对不对。题干只要求每轮写 1 个新文件，min-count 的设计假设"每题都写对、累计数正好跟上"——**一旦某道题落后（比如文件名不合规），后面每题只让新写 1 个，永远追不平，即使某一轮自己写得完全正确，checker 依然会判 -1**
  - **这是一个跟 CLI 那条诊断独立、但可能相互放大的新根因**：之前的假设是"checker ±1 是可靠的最终事实、只有中段判官可能判错"，这个假设对 `--dir --min-count` 这类题不成立——训练信号的可信度问题比之前认识到的更严重
  - 具体多少比例的 file_check 失败属于这种"这轮自己写对了、但被前面题目历史欠账连累判 FAIL"，还没查——需要 CLI 用真实 stdout 的 `found M`/`expected >= N` 核对
→ 详见 `metaclaw_migration_plan.md`"重大发现：`check_filename.py --dir --min-count` 是累计跨轮次计数"

### 当前状态（2026-08-25）

### 已就绪
**OpenClaw-RL Separate/Personal Agent Track**（同 08-13，未变）。
**MetaClaw 迁移**：同 08-21（历史状态，六处修复 + `METACLAW_TRAIN_UNTIL_DAY` + thinking 空转三处修复 + 新基线 + K=6 里程碑均已就绪），另加：[x] **暂停窗口砍断在飞生成、误判成 timeout 而不是 503 的问题已修复**（`_AGENT_PAUSE_MARKERS` 扩展，28/28 历史数据零反例支持），预期能挽回 K=6 实验里丢掉的那类样本；[x] **两项重大发现已记录，均未讨论修复方案**——中段 step-judge 与最终结果脱钩是"超长 thinking 空转"的主根因（`455a54f` 只治标）；`check_filename.py --dir --min-count` 累计计数会让"这轮做对了"的题目也被判 -1，这两条都需要下一步单独讨论怎么修。`py_compile`+合成测试验证通过（pause-marker），真实训练完全未验证，不影响正在跑的 `day12` 这条训练。

### 已知限制 / 未解决
同 08-21（历史状态），未变，另加：**`_AGENT_PAUSE_MARKERS` 扩展完全未在真实训练中验证**——合成测试只能确认代码逻辑正确，不能确认真实暂停窗口下这条新 marker 真的按预期挽回样本。**中段奖励与最终结果脱钩、`check_filename` 累计计数不公平这两项都只是诊断/发现，完全没有修复方案，更没有实现**——`455a54f` 那三处修复（next-round 反馈+`is_invalid_tool_use` 接线）不解决这两条新根因，下一轮训练如果还是崩，符合预期，不代表之前的诊断错了。

### 下一步
1. **OpenClaw-RL 复现**：同 08-17
2. **MetaClaw 迁移**：**先跟用户和 CLI 讨论这两项新发现该怎么修**（中段奖励结构、`check_filename` 累计计数的公平性），这是比"提交下一轮训练"更优先的事——不确认清楚就提交训练，很可能重复 `094611`/`day12-14` 的崩溃模式，浪费一次训练资源。用户另开默认配置训练对比 `TRAIN_UNTIL_DAY` 行为一致性的任务仍然有效，可以并行推进
3. 其余同 08-17

### 未验证
- [ ] **`METACLAW_TRAIN_UNTIL_DAY` 默认关闭时是否真的与当前 `day12` 训练行为完全一致**——用户即将验证，这次改动能不能信任的前提
- [ ] **中段奖励脱钩、`check_filename` 累计计数这两条新根因该怎么修**——方案未讨论，是当前最优先的开放问题
