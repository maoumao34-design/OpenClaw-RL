...
2. **MetaClaw 迁移**：**轨迹级样本方案已写完，等 CLI 在真实环境查证九点**（第 1 条前缀重建成功率不过则整个方案不成立）。**CLI 已用真实日志核实完 ④⑥⑧⑨**（并修正了我两处估算：samples/round 全程是 4.04 不是 2.0；轨迹中位 17~18k、全程约 13% 超 32768 硬天花板）。**①②③⑤⑦ 与 `METACLAW_*` 传播仍全开，其中 ① 前缀重建至今零证据。**安排：**直接跑满 30 天、不单独做冒烟**，内设检查点 A（day02，验管线）与检查点 B（day08–10，验长轨迹），不达标当场杀掉。见迁移文档"方案：轨迹级样本，一个 round = 一个样本"。**代码仍停在干净基线上，方案未实现。**
3. 其余同 08-17
