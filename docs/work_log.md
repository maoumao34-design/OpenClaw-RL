...
### 已就绪
（同 08-06，未变——见上方"历史状态（2026-08-06）"完整列表。另加：`STUDENT_SYSTEM_PROMPT` 的 Steps 重构（真答案/AI味两层判断+写入核实）+ 规则 4（旁路写入判负分）均已实现并本地验证；`FIRST_MESSAGE_TEMPLATE` 补丁已完全撤销，恢复论文原始措辞）

### 已知限制 / 未解决
（同 08-06，未变，见上方"历史状态（2026-08-06）"完整列表。另加：08-07 新训练暴露"格式癫痫+拒绝写文件"新失效模式，精确定位真正起始点是 P28（两个必要因子叠加：`FIRST_MESSAGE_TEMPLATE` 诱导的行为种子 + PRM 判官把"催解题/假 approve"系统性判成 +1 的打分漏洞），前者已改窄为方案 A、效果待验证，**后者（demand_solve/假 approve 不该判 +1）尚未修复，CLI 认为这条比软化措辞更接近根因**；Student 重写检测触发率随对话推进大幅下滑（原因未查）；"拒绝写文件"类失效完全没有检测规则、且可能完全逃出 PRM 打分；**新发现（暂不修）：`_build_prm_eval_prompt()` 判分规则对"催写入=隐式认可"这个信号覆盖不够精确（P4 实锤：`votes=[0,0,1]→eval_score=0.0`，应为 +1），是 P28 那条漏洞的反方向版本，从项目一开始就存在，非本次改动引入——留作后续候选修复项，见 [`issues_log.md`](issues_log.md) 2026-08-07 又一条条目）

### 下一步
1. 提交新训练，验证方案 A 是否解决"格式癫痫+拒写"问题
2. 评估是否要加"响应文本格式违规检测"规则（直接复用 Table 3 收敛判据的三个特征）
3. 评估是否要加"该行动却不行动"（拒绝调用 write/edit）检测规则
4. 其余同 08-06/07-29（见上方历史状态）

### 产出
- `scripts/prepare_openclaw_test_scripts.sh`：`FIRST_MESSAGE_TEMPLATE` 从方案 B 改回方案 A + `STUDENT_SYSTEM_PROMPT` Steps 重构（真答案/AI味两层判断 + 写入核实）
- 详细核实过程见 [`issues_log.md`](issues_log.md) 2026-08-07 三条条目

### 未验证
- [ ] 方案 A + Steps 重构组合能否避免"格式癫痫+拒写"这个新失效模式（均已实现，待真实训练验证；有明确回退判断标准，见 issues_log）
