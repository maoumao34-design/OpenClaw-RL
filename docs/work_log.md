...
### 已知限制 / 未解决
（同 08-06，未变，见上方"历史状态（2026-08-06）"完整列表。另加：08-07 新训练暴露"格式癫痫+拒绝写文件"新失效模式，已定位诱因（`FIRST_MESSAGE_TEMPLATE` 方案 B）并改窄为方案 A，但改动效果尚未用真实训练验证；Student 重写检测触发率随对话推进大幅下滑（原因未查）；满屏格式违规仍有约 37% 被误判 +1（无确定性规则兜底）；"拒绝写文件"类失效完全没有检测规则、且可能完全逃出 PRM 打分——这三项均是新发现、真实存在、尚未修复的缺口）
