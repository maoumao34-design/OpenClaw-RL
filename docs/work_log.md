...
### VPN 代理排查：git pull 一度连不上/卡死

**主要问题：**
- `git pull` 报连接代理端口失败 → sing-box 刚启动，规则集下载耗时约 167s 才真正开始监听端口，此前请求自然连不上 → 等待/重试后端口已监听
- 端口监听后 `git pull` 又卡死不动 → 出口节点（`vless[us]`）测速一直失败、转发极慢（curl 测试 20s 只收到 25KB）→ 判定是代理后端节点本身网络质量问题，与本项目代码无关，放宽 `git config http.lowSpeedLimit/lowSpeedTime` 后重试拉取成功（commit `52e9c75`）
...
### 新提交训练"Simulator 300s 不可达"排查

**主要问题：** 重新提交的训练等待外部 Simulator 时 300s 超时失败 → 一度怀疑是当天新启用的 VPN 代理环境变量污染了内部服务调用路由 → 用户自查后确认真实原因是**job 为可抢占类型，被其他任务抢占后重新排队，Simulator 服务当时并未实际运行**，与网络/代码均无关，等待重新调度后应可恢复
...
### 下一步
1. **已推送（commit `52e9c75`）、已 `git pull`、已重新提交训练**——job 为可抢占类型，目前因被抢占重新排队等待调度，待重新跑起来后观察两条新规则的真实效果
