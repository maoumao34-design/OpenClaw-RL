...
### 已就绪
（同 08-07，未变——见上方"历史状态（2026-08-07）"完整列表。另加：`STUDENT_SYSTEM_PROMPT` 四次修订——Tier-0 紧凑算式声明 + 去掉两处"keep all the steps" + criteria 段落同步修改，均已实现并本地验证）

### 已知限制 / 未解决
（同 08-07，未变，见上方"历史状态（2026-08-07）"完整列表。另加：本轮 Student 侧修复解决的是"复读为什么会发生"这条上游链路，明确不解决 tool_call XML 元循环二次崩溃、也不是给 length 加生成侧物理硬顶——这两个方向仍是待办，视这轮效果再评估）

### 下一步
1. 提交新训练，验证 Student 侧四次修订能否降低假 Tier-0 触发频率、缓解 thinking 膨胀
2. 视效果决定是否需要生成侧物理限位（frequency_penalty 等）作为补充
3. 其余同 08-07/08-06/07-29（见上方历史状态）

### 产出
- `scripts/prepare_openclaw_test_scripts.sh`：`STUDENT_SYSTEM_PROMPT` 四次修订（Tier-0 紧凑算式声明、Step 1 otherwise 分支去 keep-all-steps、criteria 段落同步）
- 详细核实过程见 [`issues_log.md`](issues_log.md) 2026-08-10 条目

### 未验证
- [ ] Student 侧四次修订能否降低假 Tier-0 触发频率、缓解"加长→重写→再加长"两拍循环（已实现，待真实训练验证）
