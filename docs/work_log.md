...
3. 确认默认配置对比通过后：（a）提交一轮带上 thinking 空转三处修复、训练继续超过 day6 的新训练，观察 day12+ 是否还会崩盘——这是验证那三处修复是否真的有效的唯一方式；（b）考虑用官方独立 `metaclaw-bench run` 重新评一次 K=6 冻结实验的 checkpoint，做严格 apples-to-apples 核实；（c）考虑用更大的 K（比如 K=12/K=20）重复冻结实验，看正面效果能不能在更长训练窗口下保持
4. 其余同 08-17
