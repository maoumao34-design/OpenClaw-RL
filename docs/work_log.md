...
此前"day09 起题题 timeout（TO=11）"是**块内出现过 `timed out` 就算**，偏松。按最终回复重数：
day09 有 5 题纯超时（其中 2 题仍 score>0）、day10 有 6 题（3 题仍 1.0）、day11 有 7 题（0）、day14 有 9 题（0）。
**day09–10 是混合态不是全废；day11 起超时占主导且不再留下可过检文件，FC 才归零。**
