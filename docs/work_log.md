...
- **修掉文档里的退格控制符**：`box` / `oxed` 在几处被写成了真正的 0x08（`` 被当转义），含 work_log 一条 08-27 的旧记录。5 份文档现已全部为 0
→ 详见 [`metaclaw_migration_plan.md`](metaclaw_migration_plan.md)「🎯 定版基线（2026-09-07）」

**主要问题（续十）：**
- **⚠️ 训练期必跟新指标：MCQ 抽取成功率。**全 run "未含 `box`" 类反馈约 **46 次**（集中 day03/07/12/26/27/30）。**格式合规是表层行为，RL 很可能几步就学会**——训练后 Acc 的提升**可能大部分来自"学会写反斜杠"而非推理能力**。**因此训练结果必须把 Acc 拆两栏报**：① MCQ 抽取成功率 ② 抽取成功者中的正确率。只报总 Acc 会把"修好格式"误当成"方法有效"
