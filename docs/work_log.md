...
- **协议偏离（2026-09-03 查清）：官方 MetaClaw 按天共用一个 session，我们从 08-19c 起是按题隔离。**因此**绝对分数不能跟论文 Table 1 或 17.8% 定版基线比**（那是协议差不是训练效果）；我们自己几次跑之间仍可比。暂不改回——改回会跟 K=6/day22 不可比，且 `safeguard` 压缩会打断前缀不变量、废掉轨迹级方案。记为待偿还的债。
- 其余同 09-02（历史状态）。
