...
- Execution Bias 全清空补丁推送后重新跑训练，Problem 36/37 起再次出现跟昨天 Problem 31 一样的 context overflow 现象；排查排除三个假设（Problem 37 内容被 36 污染、今天的 job 继承了昨天跑崩的 checkpoint、补丁没生效）后确认真实机制：模型习惯性在单个 Turn 内部连续多次用相同参数重复调用同一工具（`read`/`write`），最早在 Problem 5 就已出现，是一直存在的基线行为 → [`issues_log.md`](issues_log.md) 2026-07-24 条目
- 用统计脚本量化重复次数后发现关键规律：Problem 0-34 基本维持 1-2 次（偶尔到 5 次），**Problem 36 起出现 14-32 次的严重值**；**用户明确判断标准：10 次以内算正常，突然跳到 30 次左右这个量级才是真正导致 context 撑爆失败的异常** → [`issues_log.md`](issues_log.md) 2026-07-24 条目
- 顺带发现 Problem 35 的 `tool_calls` 用的是绝对路径（其余题目都用相对路径），导致第一版统计脚本漏统计了它
- **⚠️ 更正：** 补查后原以为"Problem 35 已经严重、真正突变起点是 35 不是 36"——这个结论是错的。用户直接核对 `simulation.log` 原始对话后发现 Problem 35 全程 4 轮均正常完成、无 overflow；反查 session UUID 对应的真实 tool_calls 后确认 **Problem 35 最长连续重复只有 7 次，且每次都被 write 打断、最终自行恢复（有界循环）**；**Problem 36 前两轮正常，是第 3 轮开始才从 10:44:01 起变成连续 32+ 次不间断的纯 read（无界循环，从未自行恢复）**——真正决定题目是否失败的是循环会不会自己终止，不是重复次数的绝对大小，且转折发生在 Problem 36 自己 session 内部的中途，不是题目之间的渐进恶化
- 可疑时间窗口同步更正：Problem 34/35（有界、能恢复）都发生在训练 step 8（10:39:03 完成）之后、step 9（10:43:42 完成）之前；**Problem 36 真正进入无界循环的时间点 10:44:01 是在 step 9 完成之后**——转折点应对应 step 9 这次权重更新，不是 step 8。仍只是时间相关性，尚未查证 step 9 对应训练 batch 是否存在具体异常样本，留作下一步排查方向
...
### 已就绪
（同 2026-07-23，未变——见上方"历史状态（2026-07-23）"完整列表。Execution Bias 全清空补丁已确认解决了当初瞄准的问题——Problem 31 那种"41 轮不收尾"的死循环没有复现；但暴露出另一个此前被它掩盖/混淆的独立问题，见下方"已知限制"）

### 已知限制 / 未解决
- **新发现：模型存在"单 Turn 内重复调用同一工具"的习惯性行为，从训练极早期（Problem 5）就已存在，通常维持在无害、有界的低量级（1-2 次，偶尔到 7-8 次，最终都能自行恢复），但从 Problem 36 起出现了性质不同的无界循环（一次都不停、连续 14-32 次，从未自行恢复）**——已确认不是 Execution Bias 章节残留内容导致（补丁确认已生效），不是 checkpoint 继承问题（今天的 job 确认从 base 模型全新开始），不是跨 session 内容污染（Problem 37 内容核对是自己真实、正确的题目内容）→ [`issues_log.md`](issues_log.md) 2026-07-24 条目
- **关键判断标准（答辩要讲清楚）：重复次数本身不是异常指标，有没有"自行终止"才是——Problem 35 那种最长 7 次、被 write 打断后能恢复的循环属于正常基线；Problem 36 从第 3 轮起进入的、连续 32+ 次纯 read 从未被打断的循环才是真正的异常**，会触发 `already_compacted_recently` 压缩失败循环、导致该题后续 turn 永久 context overflow——这是"能不能自己停下来"的质变，不是重复次数多一点少一点的量变
- Problem 36 无界循环的具体触发原因尚未查清——时间上与训练 step 9（10:43:42 完成）高度吻合（Problem 34/35 的有界循环都发生在 step 8 之后、step 9 之前；Problem 36 无界循环的起点 10:44:01 在 step 9 完成之后），但还没有查证 step 9 对应训练 batch 里是否存在具体的异常打分/异常样本
- 其余已知限制（write/overwrite 核验失败是否 Qwen3-32B 特有、"先 Separate 后 Joint"结论未经官方参考验证等）状态不变，见上方"历史状态（2026-07-23）"

### 下一步
1. **排查 Problem 36 无界循环的具体触发原因**：核对训练 step 9（及其对应的训练 batch）里的样本/打分数据，找是否存在类似 Problem 27（有缺陷的 write 被打满分）那种异常奖励事件
2. 待今天这次训练结束或停止后，用已修复的 `check_convergence.py`（配合手动 touch 占位文件）跑一次收敛结果，作为"Execution Bias 补丁后"的对比数字——但鉴于 Problem 36+ 的严重退化，这个对比数字目前可能被新发现的问题干扰，含义需要谨慎解读
3. 其余下一步同 07-23（见上方"历史状态（2026-07-23）"完整列表）
